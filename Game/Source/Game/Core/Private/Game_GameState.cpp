...
void AGame_GameState::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AGame_GameState, PresentingLockState);
}
...
void AGame_GameState::OnRep_PresentingLockState()
{
	OnPresentingLockStateUpdated.Broadcast(PresentingLockState);
}
...
bool AGame_GameState::GetIsPresentingPlayerLocked() const
{
	return PresentingLockState;
}

void AGame_GameState::SetIsPresentingPlayerLocked(bool presentingLocked)
{
	PresentingLockState = presentingLocked;

	if (GetNetMode() == NM_Standalone || GetNetMode() == NM_ListenServer)
	{
		OnRep_PresentingLockState();
	}
}
