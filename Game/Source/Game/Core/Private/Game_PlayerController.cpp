...
void AGame_PlayerController::HandleVRQuickturn(const FInputActionValue& value)
{
	if (GetWorld()->TimeSeconds - LastQuickturn > QuickturnCooldown)
	{
		...
		{
			FTransform worldTransformOverride = playerPawn->VRMovement->GetWorldTransformOverride();
		
			float deltaRotation = (value.Get<float>() > 0 ? 45 : -45) * QuickturnSpeed;
			...
			rotation = worldTransformOverride.InverseTransformRotation(rotation.Quaternion()).Rotator();
			rotation.Yaw += deltaRotation;

			UHeadMountedDisplayFunctionLibrary::ResetOrientationAndPosition(worldTransformOverride.TransformRotation(rotation.Quaternion()).Rotator().Yaw, EOrientPositionSelector::Orientation);
...
void AGame_PlayerController::QA_Doit(int32 intParam, bool bParam)
{
	/*if (UGame_TileManager* tileManager = UGame_TileManager::Get())
	{
		FGeodeticCoord2D coordinate(-79.162457, 35.122633);
		double hae;
		tileManager->GetHAE(coordinate, hae);

		UE_LOG(LogTemp, Warning, TEXT("-- hae: %f"), hae);
	}*/
	...
	attributes.Add(FXmlAttribute(TEXT("key"), TEXT("value")));
	cotEvent->AddNode(TEXT("link"), attributes);

	attributes.Empty();
	attributes.Add(FXmlAttribute(TEXT("key1"), TEXT("value1")));
	cotEvent->AddNode(TEXT("link"), attributes);

	attributes.Empty();
	attributes.Add(FXmlAttribute(TEXT("key2"), TEXT("value2")));
	cotEvent->AddNode(TEXT("link"), attributes);
