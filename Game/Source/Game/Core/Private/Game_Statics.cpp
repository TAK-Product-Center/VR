...
FString UGame_Statics::GetCoordinateStringInPreferredUnits(const FGeodeticCoord3D& coordinate, AVH_GlobeReferenceActor* globeReference)
{
	...
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		...
		{
			// MGRS (no altitude)
			return globeReference->GetGlobeCoordConverter()->GeodeticToMGRSDouble(coordinate.ToGeodeticCoord2D()).ToString();
		}
		else
		{
			// geodetic
			if (UDeveloper_Subsystem_UnitConverter* unitConverter = UDeveloper_Subsystem_UnitConverter::Get())
			{
				...
				if (desiredAltitudeUnit.Equals(TEXT("altitudeRef_MSL")))
				{
					...
					convertedAltitude = FString::Printf(TEXT("%.8Lf"), mslCoordinate.Altitude);
				}
				else
				{
					convertedAltitude = unitConverter->ConvertUnits(TEXT("altitudeRef_HAE"), FString::Printf(TEXT("%.8Lf"), altitude), desiredAltitudeUnit);
				}

				convertedAltitude = unitConverter->ConvertUnits(TEXT("meters"), convertedAltitude, desiredMeasurementUnit);

				FString heightAbbreviation = unitConverter->GetAbbreviationFromName(desiredMeasurementUnit);
				FString altitudeAbbreviation = unitConverter->GetAbbreviationFromName(desiredAltitudeUnit);

				return FString::Printf(TEXT("%.8Lf, %.8Lf, %s%s %s"), coordinate.Latitude, coordinate.Longitude, *convertedAltitude, *heightAbbreviation, *altitudeAbbreviation);
			}
		}
	}
