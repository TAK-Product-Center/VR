...
public:
	...
	UFUNCTION(BlueprintPure)
	static FString GetCoordinateStringInPreferredUnits(const FGeodeticCoord3D& coordinate, bool bIncludeAltitude);
	...
	UFUNCTION(BlueprintCallable)
	static bool IsEntityWithinPolygon(const FString& entityUID, const TArray<FGeodeticCoord2D>& polygonCoordinates);

	UFUNCTION(BlueprintPure)
	static FString MakeStringSafeForFolderName(const FString& input);

	static FString GetUniqueCOTEntityCallsign(UVH_COTEvent* cotEvent, const FString& prefix);
};
