...
#define TILE_EXTENT_D			static_cast<double>(TILE_EXTENT)
#define VECTOR_TILE_MATERIAL	TEXT("/Script/Engine.Material'/Game/VTAK/Materials/M_ShapePolyFill_VectorTile.M_ShapePolyFill_VectorTile'")
...
const FString GExtrudePropertyName			= TEXT("extrude");
...
const FString GGameSettingRoofColor			= TEXT("mapbox_vector_tile.RoofColor");
const FString GGameSettingFloorColor		= TEXT("mapbox_vector_tile.FloorColor");

int32 UGame_VectorTile_Parser::NewId = 0;
FColor UGame_VectorTile_Generator::RoofColor(255, 255, 255);
FColor UGame_VectorTile_Generator::FloorColor(127, 127, 127);
...
namespace TileUtilities
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK TileUtilities::LoadObjFromPath);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK TileUtilities::LoadMaterialFromPath);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::Parse);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::GetUniqueId);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::ring_begin);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::ring_point);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::ring_end);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::linestring_begin);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::linestring_point);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::points_begin);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Parser::UGame_VectorTile_GeometryHandler::points_point);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::Generate);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::BuildMesh);
	...
	if (UVH_MeshInfo* pMeshInfo = CreateMeshInfo())
	{
		...
		mesh.VertexColors.Reserve(vectorTile.TotalPoints);
		...
		for (; iter && (*isRunning); ++iter)
		{
			...
			if (buffer.bExtrude)
			{
				...
				mesh.VertexColors.Append(buffer.Colors);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GenerateLayer_Buildings);
	
	GetBuildingColors(RoofColor, FloorColor);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GenerateLayer_Roads);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GenerateBuilding);

	outBuffer.bExtrude	= FCString::ToBool(*feature.Properties[GExtrudePropertyName]);
	...
	if (outBuffer.bExtrude)
	{
		...
		if (*isRunning)
		{
			// This will help with building sections.  May not be 100%, but solves most cases.
			FGame_VectorTile_VertexBuffer* pBaseBuilding = nullptr;
			if (feature.Properties[GTypePropertyName].Contains(TEXT(":"))) // A colon means we have a building part of some sort
			{
				const FString* pIter = feature.Properties.Find(GBuildingIdPropertyName);
				if (pIter)
				{
					pBaseBuilding = outVertexBuffers.Find(*pIter);
					if (pBaseBuilding)
					{
						if (pBaseBuilding->bExtrude)
						{
							if (pBaseBuilding->Height > outBuffer.MinHeight)
							{
								const double baseAgl = pBaseBuilding->Height - pBaseBuilding->MinHeight;
								outBuffer.MinHeight += baseAgl;
								outBuffer.Height -= baseAgl;
							}
						}
					}
				}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GenerateBuildingIndices);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GenerateBuildingVertices);
	...
	{
		FVector vertexPos;
		floorCoordinates[i] = CalculateWorldPos(vertices[i], nw, se, outBuffer.MinHeight, vertexPos, pGlobeRef, isRunning);

		if (floorCoordinates[i].Altitude > maxFloorAltitude)
		{
			maxFloorAltitude = floorCoordinates[i].Altitude;
		}

		outBuffer.Vertices.Emplace(vertexPos);
		outBuffer.Colors.Emplace(RoofColor);
	}
	...
	for (int32 i = 0; i < nNumPoints && (*isRunning); ++i)
	{
		...
		outBuffer.Vertices[i] = pGlobeRef->GeodeticToWorldDouble(roofCoordinate);
		outBuffer.Vertices.Emplace(floorPos);
		outBuffer.Colors.Emplace(FloorColor);
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::SpawnVectorTileActor);
	...
	AsyncTask(ENamedThreads::GameThread, [meshInfo, onBuildFinishedDelegate]()
	{
		...
		if (IsValid(pWorld) && meshInfo->MeshDataList.Num() && meshInfo->MeshDataList[0].Vertices.Num() && meshInfo->MeshDataList[0].Triangles.Num())
		{
...
FGeodeticCoord3D UGame_VectorTile_Generator::CalculateWorldPos(const FVector2D& tilePos, const FGeodeticCoord2D& tileNW, const FGeodeticCoord2D& tileSE, double agl, FVector& outWorldPos, AVH_GlobeReferenceActor* globeRef, bool* isRunning)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::CalculateWorldPos);

	// Convert the local position into a latitude and longitude
	FGeodeticCoord3D vertexCoordinate;
	vertexCoordinate.Longitude = FMath::Lerp<double>(tileNW.Longitude, tileSE.Longitude, tilePos.X / TILE_EXTENT_D);
	vertexCoordinate.Latitude = FMath::Lerp<double>(tileNW.Latitude, tileSE.Latitude, tilePos.Y / TILE_EXTENT_D);
	vertexCoordinate.Altitude = agl;
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::CreateMeshInfo);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::CalculateTileBounds);
...
void UGame_VectorTile_Generator::GetBuildingColors(FColor& outRoofColor, FColor& outFloorColor)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GetBuildingColors);

	outRoofColor = FColor(255, 255, 255);
	outFloorColor = FColor(127, 127, 127);

	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		// Roof Color
		const FString sRoofColor = gameSettings->GetSettingString(GGameSettingRoofColor);
		if (!outRoofColor.InitFromString(sRoofColor))
		{
			outRoofColor = FColor(255, 255, 255);
		}

		// Floor Color
		const FString sFloorColor = gameSettings->GetSettingString(GGameSettingFloorColor);
		if (!outFloorColor.InitFromString(sFloorColor))
		{
			outFloorColor = FColor(127, 127, 127);
		}
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_VectorTile_Generator::GetGlobeRef);
	...
	bIsRunning(false)
{
	...
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		gameSettings->StoreDefaultSettingString(GGameSettingRoofColor, FColor::White.ToString());
		gameSettings->StoreDefaultSettingString(GGameSettingFloorColor, FColor(127, 127, 127).ToString());
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::Init);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::Run);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::Stop);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::Exit);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::QueueTile);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::StartThread);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK Game_VectorTile_Task::StopThread);
