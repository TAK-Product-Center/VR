...
bool UGame_MeshComponent_DTED::GetHeightData(TArray<double>& haeValues, const FGeodeticCoord2D& northWestBound, const FGeodeticCoord2D& southEastBound, double postSpacingMeters, int32& totalLongitudePosts, int32& totalLatitudePosts)
{
	using namespace TAK::Engine::Formats::DTED;
	using namespace TAK::Engine::Elevation;
	using namespace TAK::Engine::Feature;
	...
	if (GetComponentStatus() == EComponentStatus::Initialized)
	{
		...
		if (Vertices.Num() == 0)
		{
			Async(EAsyncExecution::ThreadPool, [this]()
			{
				...
				if (GetHeightData(haeValues, GetNorthWestBound(), GetSouthEastBound(), postSpacingMeters, totalLongitudePosts, totalLatitudePosts))
				{
