...
APlayerState* UGame_BloodhoundManager::IsAPlayer(const FString& entityUID) const
{
...
bool UGame_BloodhoundManager::HasLocalPlayer(const FBloodhoundUIDRelation& relation) const
{
	if (AGame_PlayerState* playerState = UGame_Statics::GetLocalPlayerState())
	{
		FString playerID = FString::Printf(TEXT("%d"), playerState->GetPlayerId());

		return relation.ChaserUID.Equals(playerID) || relation.TargetUID.Equals(playerID);
	}
...
bool UGame_BloodhoundManager::EntitiesArePagedIn(AGame_VirtualObjective* virtualObjective, const FBloodhoundUIDRelation& relation) const
{
	...
	bool bChaserIsPlayer = IsAPlayer(relation.ChaserUID) != nullptr;
	bool bTargetIsPlayer = IsAPlayer(relation.TargetUID) != nullptr;
...
void UGame_BloodhoundManager::UpdateBloodhoundActors(const FBloodhoundUIDRelation& relation, bool bIsInTOC)
{
	...
	{
		// if bloodhound has local player, do not allow in TOC and destroy existing bloodhound actors
		if (HasLocalPlayer(relation))
		{
			if (bIsInTOC)
			{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("-- spawning bloodhound | %s | %s"), *relation.BloodhoundUID, *relation.ChaserUID);
		...
		if (bBloodhoundActorExists && !EntitiesArePagedIn(virtualObjective, relation))
		{
			UE_LOG(LogTemp, Warning, TEXT("-- despawning bloodhound | %s | %s"), *relation.BloodhoundUID, *relation.ChaserUID);
			virtualObjective->PageOutEntity(relation.BloodhoundUID);
		}
	}
}
...
void UGame_BloodhoundManager::GetBoundingBox(const FString& chaserEntityUID, const FString& targetEntityUID, FGeodeticCoord2D& northWestBB, FGeodeticCoord2D& southEastBB) const
{
	...
	if (entityDB != nullptr && handlerService != nullptr)
	{
		...
		if (IsAPlayer(chaserEntityUID) == nullptr)
		{
		...
		if (IsAPlayer(targetEntityUID) == nullptr)
		{
