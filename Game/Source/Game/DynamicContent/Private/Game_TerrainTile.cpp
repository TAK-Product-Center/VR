	...
	HighResCooldown(0.01)
{
	...
	KdTree = NewObject<UGame_KdTree_Terrain>();
}
...
{
	if (KdTree->IsBuilding())
	{
		// Wait until the KD Tree is finished building before moving on to prevent threading related crashes.
		KdTree->Stop();
		FPlatformProcess::ConditionalSleep([this]() { return KdTree->IsReady(); });
	}
...
void AGame_TerrainTile::BuildKdTree()
{
	KdTree->Append(Vertices, Triangles);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VR-TAK UGame_TileManager::HandleMeshGameLogicCompleted);
...
bool AGame_TerrainTile::RayTrace(const FVector start, const FVector& dir, FVector& outHit)
{
	return KdTree->LineTrace(start, dir, outHit);
}

bool AGame_TerrainTile::IsWorldPartioningReady() const
{
	return KdTree->IsReady();
}
...
bool AGame_TerrainTile::IsCoordInBounds(const FGeodeticCoord2D& coord) const
{
	// 1) Latitude test (note: nw.Lat > se.Lat)
	if (coord.Latitude > TileBounds.Key.Latitude || coord.Latitude < TileBounds.Value.Latitude)
	{
		return false;
	}
;
    // 2) Normalize all longitudes
	double lon = NormalizeLon(coord.Longitude);
	double west = NormalizeLon(TileBounds.Key.Longitude);		// Western edge
	double east = NormalizeLon(TileBounds.Value.Longitude);	// Eastern edge

	// 3) Longitude Test
	if (west <= east)
	{
		return lon >= west && lon <= east; // No wrap: interval [west..east]
	}
	else
	{
		
		return lon >= west || lon <= east; // Wrap-around: [west..180) U (-180..east]
	}
}
