		...
		Async(EAsyncExecution::ThreadPool, [this]()
		{
			...
			if (!bOffline && !bXMLLoaded)
			{
				...
				if (!bOffline)
				{
					...
					for (int i = 0; i < potentialFilePaths.Num(); i++)
					{
...
void UGame_MatComponent_XML::CreateMat()
{
	...
	Async(EAsyncExecution::ThreadPool, [this]()
	{
		...
		if (!bFoundTile)
		{
			if (Attempt != EAttempt::TMS_Y_Reversed)
			{
...
cv::Mat UGame_MatComponent_XML::CreateMatOfAllTiles(int32 pixelsX, int32 pixelsY)
{
	...
	ParallelFor(endY - startY + 1, [this, &allTiles, startY, endY, startX, endX](int32 y) 
	{
		...
		ParallelFor(endX - startX + 1, [this, &allTiles, y, endY, startX, endX](int32 x)
		{
			x += startX;
			...
			int32 yPos = (y - NorthWestTile.Value) * (PixelHeight - 1);
