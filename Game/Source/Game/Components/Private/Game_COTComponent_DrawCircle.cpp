...
void UGame_COTComponent_DrawCircle::EndPlay(const EEndPlayReason::Type endPlayReason)
{
	...
	for (int32 cylinderMeshIndex = 0; cylinderMeshIndex < CylinderMeshes.Num(); cylinderMeshIndex++)
	{
		if (CylinderMeshes[cylinderMeshIndex] != nullptr)
		{
			CylinderMeshes[cylinderMeshIndex]->Destroy();
		}
	}
}
...
void UGame_COTComponent_DrawCircle::PostModifyVOGlobeReference(const TArray<FVector>& updatedPositions)
{
	...
	for (int32 cylinderMeshIndex = 0; cylinderMeshIndex < CylinderMeshes.Num(); cylinderMeshIndex++)
	{
		CylinderMeshes[cylinderMeshIndex]->SetActorLocation(updatedPositions.Last());
	}
...
void UGame_COTComponent_DrawCircle::SyncPerimeterEditPoints()
{
	...
	{
		unitsPerMeter = virtualObjective->GetActorScale3D().X * 100.0;
	}
...
void UGame_COTComponent_DrawCircle::SyncCylinderMeshes()
{
	for (int32 i = 0; i < CylinderMeshes.Num(); i++)
	{
		CylinderMeshes[i]->Destroy();
	}
	CylinderMeshes.Empty();
	...
	{
		UE_LOG(Game, Warning, TEXT("-- UGame_COTComponent_DrawCircle::SyncCylinderMeshes | globeReference is nullptr | aborting"));
	...
	for (int32 keysIndex = 0; keysIndex < outKeys.Num(); keysIndex++)
	{
		AGame_SimpleSegmentedLineActor* circle_Line = outKeys[keysIndex];
		if (circle_Line != nullptr)
		{
			quadVertices.Empty();

			bool newMesh = false;
			if (CylinderMeshes.Num() < keysIndex + 1)
			{
				...
				AGame_Actor_Mesh* newMeshActor = GetWorld()->SpawnActor<AGame_Actor_Mesh>(CylinderFillActorClass, spawnTransform, spawnParams);

				CylinderMeshes.Add(newMeshActor);
				newMesh = true;
			}

			if (CylinderMeshes[keysIndex] != nullptr)
			{
				CylinderMeshes[keysIndex]->SetActorScale3D(FVector::OneVector);

				TArray<AGame_EditPoint*> editPoints = Lines[circle_Line]->GetEditPoints();
				...
				UMaterialInstanceDynamic* dynMaterial = Cast<UMaterialInstanceDynamic>(CylinderMeshes[keysIndex]->ProceduralMesh->GetMaterial(0));
				...
				CylinderMeshes[keysIndex]->BuildMeshFromVerticesAndTriangles(vertices, triangles, dynMaterial);
				
				CylinderMeshes[keysIndex]->AttachToActor(CenterEditPoint, FAttachmentTransformRules::KeepWorldTransform);
				CylinderMeshes[keysIndex]->SetActorRelativeLocation(FVector::ZeroVector, false);
			}
		}
	}
}
...
void UGame_COTComponent_DrawCircle::SyncColors()
{
	...
	for (AGame_Actor_Mesh* cylinderMesh : CylinderMeshes)
	{
...
void UGame_COTComponent_DrawCircle::DetachCylindersFromCenter()
{
	...
	for (int32 cylinderMeshIndex = 0; cylinderMeshIndex < CylinderMeshes.Num(); cylinderMeshIndex++)
	{
		CylinderMeshes[cylinderMeshIndex]->DetachFromActor(detachRules);
	}
}
...
void UGame_COTComponent_DrawCircle::AttachEditPointsToCenter()
{
	...
	for (int32 cylinderMeshIndex = 0; cylinderMeshIndex < CylinderMeshes.Num(); cylinderMeshIndex++)
	{
		CylinderMeshes[cylinderMeshIndex]->AttachToActor(CenterEditPoint, attachmentRules);
	}
}
