...
void UGame_UserWidget_Arrow::Update(const FVector& scale3D, const AGame_EditPoint* baseEditPoint, const AGame_EditPoint* arrowEditPoint)
{
	...
	{
		// update RangeMeters
		float rangeMeters = FVector::Distance(arrowEditPoint->GetActorLocation(), baseEditPoint->GetActorLocation()) / 100.f / scale3D.X;
		...
		FRotator worldRotation = (arrowEditPoint->GetActorLocation() - baseEditPoint->GetActorLocation()).GetSafeNormal().Rotation();

		if (VirtualObjective)
		{
			if (AVH_GlobeReferenceActor* globeRef = VirtualObjective->GetGlobeReference())
			{
				float bearingDegrees = globeRef->WorldRotationToBearing(worldRotation);
				if (bearingDegrees != BearingDegrees)
				{
