...
#include "UI_Subsystem_Core_Settings.h"
...
void UGame_UserWidget_InterfaceSettings::NativeConstruct()
{
	...
	{
		FVector defaultOffset = subsystem->DefaultWorldSettings.LocalSpawnOffset;

		for (const TPair<TSubclassOf<UUserWidget>, FOpenUserWidgetWorldSettings>& entry : subsystem->DefaultClassWorldSettings)
		{
			if (entry.Value.LocalSpawnOffset.Equals(defaultOffset))
			{
				UserWidgetClassesForOffset.Add(entry.Key);
			}
		}
	}
}
...
void UGame_UserWidget_InterfaceSettings::SetVRUIOffset(const FVector& offset)
{
	if (UUI_Subsystem_Core* subsystem = UUI_Subsystem_Core::Get())
	{
		// default
		FOpenUserWidgetWorldSettings settings = subsystem->GetUserWidgetClassWorldSettings(nullptr);
		settings.LocalSpawnOffset = offset;

		subsystem->SetUserWidgetClassWorldSettings(nullptr, settings);

		if (UUI_Subsystem_Core_Settings* coreSettings = GetMutableDefault<UUI_Subsystem_Core_Settings>())
		{
			coreSettings->DefaultWorldSettings = subsystem->DefaultWorldSettings;
			coreSettings->SaveConfig();
		}

		// specified classes
		for (TSubclassOf<UUserWidget> userWidgetClass : UserWidgetClassesForOffset)
		{
			settings = subsystem->GetUserWidgetClassWorldSettings(userWidgetClass);
			settings.LocalSpawnOffset = offset;

			subsystem->SetUserWidgetClassWorldSettings(userWidgetClass, settings);
		}

		if (UUI_Subsystem_Core_Settings* coreSettings = GetMutableDefault<UUI_Subsystem_Core_Settings>())
		{
			coreSettings->DefaultClassWorldSettings = subsystem->DefaultClassWorldSettings;
			coreSettings->SaveConfig();
		}
	}
}
	
void UGame_UserWidget_InterfaceSettings::SetVRRibbonOffset(const FVector& offset)
{
	if (UUI_Subsystem_Core* subsystem = UUI_Subsystem_Core::Get())
	{
		FOpenUserWidgetWorldSettings settings = subsystem->GetUserWidgetClassWorldSettings(RibbonClass);
		settings.LocalSpawnOffset = offset;

		subsystem->SetUserWidgetClassWorldSettings(RibbonClass, settings);
...
void UGame_UserWidget_InterfaceSettings::SetVRBeamEndVisual(bool bEnabled, double scale, const FLinearColor& color)
{
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		gameSettings->StoreSettingBool(TEXT("bShowBeamEndMesh"), bEnabled);
		
		gameSettings->StoreSettingDouble(TEXT("BeamEndMeshScale"), scale);

		FLinearColor withAlpha = color;
		withAlpha.A = 0.25;

		gameSettings->StoreSettingString(TEXT("BeamEndMeshColor"), withAlpha.ToString());
	}
}

void UGame_UserWidget_InterfaceSettings::ResetVRUIOffset()
{
	FVector defaultOffset(400, 0, 0);

	SetVRUIOffset(defaultOffset);

	ReceiveSetVRUIOffset(defaultOffset);
}

void UGame_UserWidget_InterfaceSettings::ResetVRRibbonOffset()
{
	FVector defaultOffset(450, 0, 75);

	SetVRRibbonOffset(defaultOffset);

	ReceiveSetVRRibbonOffset(defaultOffset);
}

void UGame_UserWidget_InterfaceSettings::ResetVRBeamEndVisual()
{
	SetVRBeamEndVisual(false, 1, FLinearColor(1, 1, 0, 0.25));

	ReceiveSetVRBeamEndVisual(false, 1, FLinearColor(1, 1, 0, 0.25));
}
...
void UGame_UserWidget_InterfaceSettings::BindDelegates()
{
	...
	{
		ApplicationScaleEditableText->GetEditableText()->OnTextCommitted.AddUniqueDynamic(this, &UGame_UserWidget_InterfaceSettings::HandleApplicationScaleTextEdited);
	}
...
void UGame_UserWidget_InterfaceSettings::ResetApplicationScale()
{
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		...
		ApplicationScaleEditableText->GetEditableText()->SetText(FText::FromString(FString::Printf(TEXT("%f"), applicationScaleDefault)));
...
void UGame_UserWidget_InterfaceSettings::SetInitialWidgetValues()
{
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		if (IsValid(CenterOnBeamCheckBox))
		...
		if (IsValid(ComboBoxKeyboard))
		{
			...
			if (UGame_Subsystem_Keyboard* subsystem = UGame_Subsystem_Keyboard::Get())
			{
				for (const TPair<FString, TSubclassOf<AActor>>& pair : subsystem->KeyboardClassMap)
				{
					ComboBoxKeyboard->AddOption(pair.Key);
				}

				ComboBoxKeyboard->SetSelectedOption(gameSettings->GetSettingString(TEXT("Keyboard")));
			}
		}
		...
		if (IsValid(ApplicationScaleSlider) && IsValid(ApplicationScaleEditableText))
		{
			...
			ApplicationScaleEditableText->GetEditableText()->SetText(FText::FromString(FString::Printf(TEXT("%f"), applicationScale)));
		}

		FLinearColor beamEndColor;
		beamEndColor.InitFromString(gameSettings->GetSettingString(TEXT("BeamEndMeshColor")));

		ReceiveSetVRBeamEndVisual(gameSettings->GetSettingBool(TEXT("bShowBeamEndMesh")), gameSettings->GetSettingDouble(TEXT("BeamEndMeshScale")), beamEndColor); 
	}

	if (UUI_Subsystem_Core* subsystem = UUI_Subsystem_Core::Get())
	{
		FOpenUserWidgetWorldSettings settings = subsystem->GetUserWidgetClassWorldSettings(RibbonClass);
		ReceiveSetVRRibbonOffset(settings.LocalSpawnOffset);

		settings = subsystem->GetUserWidgetClassWorldSettings(nullptr);
		ReceiveSetVRUIOffset(settings.LocalSpawnOffset);
	}
}
...
void UGame_UserWidget_InterfaceSettings::RestorePreviousApplicationScale()
{
	...
	ApplicationScaleEditableText->GetEditableText()->SetText(FText::FromString(FString::Printf(TEXT("%f"), PreviousApplicationScale)));
...
void UGame_UserWidget_InterfaceSettings::HandleComboBoxKeyboardSelectionChanged(FString selected, ESelectInfo::Type selectedType)
{
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		if (UGame_Subsystem_Keyboard* subsystem = UGame_Subsystem_Keyboard::Get())
		{
			if (subsystem->KeyboardClassMap.Contains(selected))
			{
...
void UGame_UserWidget_InterfaceSettings::HandleApplicationScaleTextEdited(const FText& text, ETextCommit::Type commitMethod)
{
...
void UGame_UserWidget_InterfaceSettings::HandleKeyboardSpawnDestroyed(bool bSpawn, AActor* keyboardContainer)
{
	if (!bSpawn)
	{
		HandleApplicationScaleTextEdited(ApplicationScaleEditableText->GetEditableText()->GetText(), ETextCommit::OnEnter);
	}
}
