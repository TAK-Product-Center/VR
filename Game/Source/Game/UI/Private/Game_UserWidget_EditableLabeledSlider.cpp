...
void UGame_UserWidget_EditableLabeledSlider::SetValue(float newValue, bool saveToDB)
{
	...
	if (IsValid(ValueEditableText))
	{
		...
		ValueEditableText->SetText(FText::AsNumber(newValue, &numberFormatOptions), false);
	}
...
void UGame_UserWidget_EditableLabeledSlider::BindDelegates()
{
	...
	if (IsValid(ValueEditableText))
	{
		ValueEditableText->OnEntrySubmittedCallback.AddUniqueDynamic(this, &UGame_UserWidget_EditableLabeledSlider::HandleValueTextEdited);
	}
}
...
void UGame_UserWidget_EditableLabeledSlider::HandleValueTextEdited(FString inString)
{
