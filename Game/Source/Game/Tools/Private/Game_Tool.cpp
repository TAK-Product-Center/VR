	...
	DefaultBeamColor(FLinearColor::Green)
{
	...
	MainMesh->CanCharacterStepUpOn = ECanBeCharacterBase::ECB_No;
	...
	BeamMesh->CanCharacterStepUpOn = ECanBeCharacterBase::ECB_No;
	BeamMesh->SetGenerateOverlapEvents(false);

	CameraScalingBeamEndMesh = CreateDefaultSubobject<UGame_CameraScalingComponent>(TEXT("ScalingBeamEndMesh"));
	CameraScalingBeamEndMesh->SetupAttachment(rootComponent);
	CameraScalingBeamEndMesh->MinMaxCameraDistanceForScalingTOC = FVector2D(20000, 13106800);
	CameraScalingBeamEndMesh->MinScaleTOC = FVector(0.015, 0.015, 0.015);
	CameraScalingBeamEndMesh->MaxScaleTOC = FVector(10, 10, 10);
	CameraScalingBeamEndMesh->MinMaxCameraDistanceForScaling = FVector2D(1000, 65534);
	CameraScalingBeamEndMesh->MinScale = FVector(0.1, 0.1, 0.1);
	CameraScalingBeamEndMesh->MaxScale = FVector(10, 10, 10);
	CameraScalingBeamEndMesh->PrimaryComponentTick.TickInterval = 0;

	BeamEndMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("BeamEndMesh"));
	BeamEndMesh->SetupAttachment(CameraScalingBeamEndMesh);
	BeamEndMesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);
	BeamEndMesh->SetOnlyOwnerSee(true);
	BeamEndMesh->CanCharacterStepUpOn = ECanBeCharacterBase::ECB_No;
	BeamEndMesh->SetGenerateOverlapEvents(false);
...
void AGame_Tool::RemovedFromInputToolStack_Implementation(UVR_MotionControllerComponent* motionControllerComponent)
{
	...
	// close DefaultOptionsUserWidget if other tool on top of input stack is not the same class
	if (IsValid(DefaultOptionsUserWidget))
	{
		if (AGame_Pawn* playerPawn = UGame_Statics::GetVHPlayerPawn())
		{
			bool bCloseDefaultUI = true;

			AActor* tool = playerPawn->LeftController->GetInputToolStackTop();
			if (tool != this)
			{
				if (tool->GetClass()->IsChildOf(GetClass()))
				{
					bCloseDefaultUI = false;
				}
			}

			if (bCloseDefaultUI)
			{
				tool = playerPawn->RightController->GetInputToolStackTop();
				if (tool != this)
				{
					if (tool->GetClass()->IsChildOf(GetClass()))
					{
			...
			if (bCloseDefaultUI)
			{
				...
				{
					subsystem->CloseUserWidget(DefaultOptionsUserWidget);
				}
			}
		}
	}
...
void AGame_Tool::ActionInput_Implementation(const FInputActionInstance& inputActionInstance)
{
	...
	if (action.Equals(TEXT("Tool_WorldUI_Zoom"), ESearchCase::IgnoreCase) && DraggedWorldUIActor != nullptr)
	{
		FVector interactionLocation;
		if (GetWorldUIHitLocation(interactionLocation))
		{
			FTransform toolTransform = GetActorTransform();

			FVector interactionLocal = toolTransform.InverseTransformPosition(interactionLocation);
			
			// > 0 means UI is moving away from player
			// < 0 means UI is moving towards player
			float zoomAmount = inputActionInstance.GetValue().Get<float>();

			// do not allow movement towards tool if too close
			if ((zoomAmount > 0 && interactionLocal.Size() < 3000) ||
				(zoomAmount < 0 && interactionLocal.Size() > 250))
			{
				FVector uiLocal = toolTransform.InverseTransformPosition(DraggedWorldUIActor->GetActorLocation());

				FVector diff = interactionLocal - uiLocal;

				FVector desiredInteractionLocal = FMath::Lerp<FVector>(interactionLocal, FVector::Zero(), zoomAmount > 0 ? -0.025 : 0.025);
			
				DraggedWorldUIActor->SetActorLocation(GetActorTransform().TransformPosition(desiredInteractionLocal - diff));
			}
		}
	}
	else if (action.Equals(TEXT("Tool_GlobeZoom"), ESearchCase::IgnoreCase) && DraggedWorldUIActor == nullptr)
	{
...
void AGame_Tool::BeginPlay()
{
	...
	{
		if (BeamMesh->GetStaticMesh() != nullptr)
		{
			BeamMaterial = BeamMesh->CreateDynamicMaterialInstance(0);

			if (IsValid(BeamMaterial))
			{
				BeamMaterial->SetVectorParameterValue(TEXT("Color"), BeamColor);
			}
	...
	{
		if (BeamEndMesh->GetStaticMesh() != nullptr)
		{
			BeamEndMesh->CreateDynamicMaterialInstance(0);
		
			HandleBoolSettingStored(TEXT("bShowBeamEndMesh"), gameSettings->GetSettingBool(TEXT("bShowBeamEndMesh")));

			HandleDoubleSettingStored(TEXT("BeamEndMeshScale"), gameSettings->GetSettingDouble(TEXT("BeamEndMeshScale")));
		
			HandleStringSettingStored(TEXT("BeamEndMeshColor"), gameSettings->GetSettingString(TEXT("BeamEndMeshColor")));

			gameSettings->OnBoolSettingStored.AddDynamic(this, &AGame_Tool::HandleBoolSettingStored);
			gameSettings->OnDoubleSettingStored.AddDynamic(this, &AGame_Tool::HandleDoubleSettingStored);
			gameSettings->OnStringSettingStored.AddDynamic(this, &AGame_Tool::HandleStringSettingStored);
		}
	}
...
bool AGame_Tool::StartDraggingWorldUI(UUserWidget* userWidget, AActor* worldUIActor)
{
	...
	{
		DraggedWorldUIActor = worldUIActor;
...
void AGame_Tool::StopDraggingWorldUI(UUserWidget* userWidget, AActor* worldUIActor)
{
	DraggedWorldUIActor = nullptr;
}
...
bool AGame_Tool::IsDraggingWorldUI() const
{
	return IsValid(DraggedWorldUIActor);
}

bool AGame_Tool::IsPanning() const
{
	return PanningState == EPanningState::Panning;
}
...
bool AGame_Tool::ShouldUpdateBeam()
{
	...
	if (netMode == NM_DedicatedServer || !IsLocallyOwned() || BeamMesh->GetStaticMesh() == nullptr)
	{
		return false;
	}
...
void AGame_Tool::TickUpdateBeam(float deltaSeconds)
{
	if (ShouldUpdateBeam())
	{
		...
		FVector beamEndLocation = VRTrace->GetComponentTransform().TransformPosition(FVector(localTraceDistance, 0, 0));
		CameraScalingBeamEndMesh->SetWorldLocation(beamEndLocation);
		CameraScalingBeamEndMesh->SetWorldRotation(VRTrace->GetComponentTransform().TransformRotation(FRotator(90, 0, 0).Quaternion()));
	...
	if (PanningState == EPanningState::Panning)
	{
		...
		else
		{
			...
			if (!bSkipEarthPan)  
			{
				...
				playerPawn->Camera->SetQuickTurn(0);

				FTransform transform = playerPawn->Camera->GetCameraTransformOverride();
				transform.SetRotation(rotationToEarthCenter);

				playerPawn->Camera->SetCameraTransformOverride(transform, true);
			}
...
void AGame_Tool::SyncToToolVisualization(const SpatialiteResult& result)
{
	...
	BeamEndMesh->SetTranslucentSortPriority(100);
}
...
void AGame_Tool::HandleBoolSettingStored(const FString& settingName, bool bValue)
{
	if (settingName.Equals(TEXT("bShowBeamEndMesh")))
	{
		if (UDeveloper_Subsystem_Core* subsystem = UDeveloper_Subsystem_Core::Get())
		{
			BeamEndMesh->SetHiddenInGame(!bValue || subsystem->GetHardwareInterfaceMode() == EHardwareInterfaceMode::Desktop);
		}
	}
}

void AGame_Tool::HandleDoubleSettingStored(const FString& settingName, double value)
{
	if (settingName.Equals(TEXT("BeamEndMeshScale")))
	{
		BeamEndMesh->SetRelativeScale3D(FVector(value, value, 1));
	}
}

void AGame_Tool::HandleStringSettingStored(const FString& settingName, const FString& value)
{
	if (settingName.Equals(TEXT("BeamEndMeshColor")))
	{
		if (UMaterialInstanceDynamic* mid = Cast<UMaterialInstanceDynamic>(BeamEndMesh->GetMaterial(0)))
		{
			FLinearColor color;
		...
		if (AGame_VirtualObjective* playerVO = AGame_PlayerController::GetPlayerActiveVO())
		{
			if (AGame_GlobeReferenceActor_Ellipsoid* globeReferenceEllipsoid = Cast<AGame_GlobeReferenceActor_Ellipsoid>(playerVO->GetGlobeReference()))
			{
				if (UGame_Statics::GetPlayerWorldTransformOverride().Equals(FTransform::Identity))
				{
					...
					{
						// use tool rotation and location if did not hit terrain
