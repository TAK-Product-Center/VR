...
namespace
{
    ...
    TAKErr createHeightmapImpl(double* value, ElevationChunkCursor& cursor, const HeightmapParams& params, const bool clear) NOTHROWS;
}
...
{
    return createHeightmapImpl(value, cursor, params, true);
}
...
namespace
{
    ...
    TAKErr createHeightmapImpl(double* value, const double gsd, const HeightmapParams& params, const HeightmapStrategy strategy, T sourceQuery) NOTHROWS
    {
        ...
        code = Polygon2_fromEnvelope(queryParams.spatialFilter, params.bounds);
        TE_CHECKRETURN_CODE(code);
        ...
        for(std::size_t i = 0u; i < numQueries; i++) {
            ...
            if(code != TE_Ok)
                continue;
            code = createHeightmapImpl(value, *result, params, (i==0u));
            if(code == TE_Ok)
                return code;
        }
        return TE_Done;
    }
    TAKErr createHeightmapImpl(double* value, ElevationChunkCursor &cursor, const HeightmapParams &params, const bool init) NOTHROWS
    {
        TAKErr code(TE_Ok);
