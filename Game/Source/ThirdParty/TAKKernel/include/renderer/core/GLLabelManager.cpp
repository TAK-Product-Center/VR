...
bool hasFont(const StringMap<StringMap<StringSet<TAK::Engine::Port::StringLess>>>& typeFontStyle, const char* font, const char* style, const char* type) NOTHROWS
{
    auto fonts = typeFontStyle.find(type);
    if (fonts == typeFontStyle.end())
        return false;
    auto styles = fonts->second.find(font);
    if (styles == fonts->second.end())
        return false;
    return (styles->second.find(style) != styles->second.end());
}
...
void GLLabelManager::resetFont() NOTHROWS {
    ...
    defaultFontName = "";
    for (auto types : loadedFonts_) {
        for (auto fonts : types.second) {
            for (auto style : fonts.second) {
                fontsToLoad_[types.first][fonts.first].insert(style);
            }
        }
    }
    loadedFonts_.clear();
    for (auto types : invalidFonts_) {
        for (auto fonts : types.second) {
            for (auto style : fonts.second) {
                fontsToLoad_[types.first][fonts.first].insert(style);
            }
...
uint32_t GLLabelManager::addLabel(GLLabel& label) NOTHROWS {
    Monitor::Lock lock(monitor_);
    auto opts = toGlyphBuffersOpts(label);
    if (!opts.fontName || !opts.style) {
        opts = toGlyphBuffersOpts(label);
    }

    if(!hasFont(loadedFonts_, opts.fontName, opts.style, "msdf") && !hasFont(invalidFonts_, opts.fontName, opts.style, "msdf"))
        fontsToLoad_["msdf"][opts.fontName.get()].insert(opts.style.get());
...
void GLLabelManager::setTextFormat(const uint32_t id, const TextFormatParams* fmt) NOTHROWS {
    ...
    updateLabel(id, [&](GLLabel& lbl)
    {
        ...
        auto opts = toGlyphBuffersOpts(lbl);
        if(!hasFont(loadedFonts_, opts.fontName, opts.style, "msdf") && !hasFont(invalidFonts_, opts.fontName, opts.style, "msdf"))
            fontsToLoad_["msdf"][opts.fontName.get()].insert(opts.style.get());
        if (labels.locked)
            labels.modifyBuffer.replaceLabels = true;
...
void GLLabelManager::draw(const GLGlobeBase& view, const int render_pass) NOTHROWS {
    ...
    std::shared_ptr<GLGlyphBatchFactory> factory;
    {
        Monitor::Lock lock(monitor_);
        if (!context_)
        ...
        if (factory) {
            ...
            for (const auto& types : fontsToLoad_) {
                for (const auto& fonts : types.second) {
                    for (const auto& style : fonts.second) {
                        loadAtlas(*factory, fonts.first, style, types.first);
                    }
                }
            }
            if (!fontsToLoad_.empty())
                glyphFactoryVersion_++;
            fontsToLoad_.clear();
        }
    }
        ...
        if (always_render_idx_ != NO_ID && labels_.find(always_render_idx_) != labels_.end()) {
            ...
            if (forRenderPass && geometry != nullptr) {
                ...
                for(auto &placement : label.transformed_anchor_) {
                    bool didReplace;
                    if(label.place(placement, view, rtf, label_placements, GLLabel::PlacementDeconflictMode::None, didReplace))
...
void GLLabelManager::draw(const GLGlobeBase& view, const std::vector<uint32_t> &idsToRender,
    ...
    for (auto it = ids.begin(); it != ids.end(); it++) {
        ...
        for(auto &placement : label.transformed_anchor_)
        {
            ...
            if (replace_labels_)
            {
                if(label.place(placement, *view.renderPass, rtf, label_placements, deconflict, didReplace))
                {
                    if(!didReplace)
                    {
        ...
        if(!didReplace && canDraw)
        {
            ...
            if (replace_labels_ && deconflict == GLLabel::PlacementDeconflictMode::None) {
    ...
    if (!nodeconflict_labels.empty()) {
        ...
        for (int label_id : nodeconflict_labels)
        {
            ...
            for (auto& placement : label.transformed_anchor_) {
                ...
                bool didReplace;
                if (label.place(placement, view, rtf, nodeconflict_placements, GLLabel::PlacementDeconflictMode::Discard, didReplace)) {
    ...
    for(int label_id : replaced_labels)
    {
        ...
        for(auto &placement : label.transformed_anchor_) {
            bool didReplace;
            if(label.place(placement, view, rtf, label_placements, GLLabel::PlacementDeconflictMode::Shift, didReplace)) {
                label_placements.push_back(placement);
            }
        }
        if (fallback_method) {
...
std::shared_ptr<GLGlyphBatchFactory> GLLabelManager::defaultGlyphBatchFactory() NOTHROWS {
    Monitor::Lock lock(monitor_);
    if (glyph_batch_factory_ == nullptr && context_ && context_->isRenderThread()) {
        glyph_batch_factory_ = std::make_shared<GLGlyphBatchFactory>();
...
TAKErr GLLabelManager::loadAtlas(GLGlyphBatchFactory& factory, const char* font, const char* style, const char* type) NOTHROWS {
    ...
    if (hasFont(loadedFonts_, font, style, type))
        return TE_Ok;
    if (hasFont(invalidFonts_, font, style, type))
        return TE_InvalidArg;
    if (legacyFonts_.find(font) != legacyFonts_.end())
        return TE_InvalidArg;

    TAK::Engine::Port::String fontAtlasUri;
    code = ConfigOptions_getOption(fontAtlasUri, "default-font-atlas-uri");
    if (code != TE_Ok) {
        // Fall back to legacy default-font-atlas-path
        TAK::Engine::Port::String fontAtlasPath;
        code = ConfigOptions_getOption(fontAtlasPath, "default-font-atlas-path");
        if (code == TE_Ok) {
            std::ostringstream strm;
            strm << "file://" << fontAtlasPath;
            fontAtlasUri = strm.str().c_str();
        } else {
            // Finally fall back to trying assets URI
            fontAtlasUri = "asset://fonts";
        }
    }
    ...
    code = GlyphAtlas_open(atlas, fontAtlasUri, font, style, type, &opts);
    if (code == TE_Ok && atlas) {
        TAK::Engine::Renderer::BitmapPtr bitmap(nullptr, nullptr);
        code = GlyphAtlas_loadBitmap(bitmap, fontAtlasUri, font, style, type, &opts);
        if (code == TE_Ok && bitmap) {
            std::unique_ptr<GLTexture2> tex = std::unique_ptr<GLTexture2>(new GLTexture2(bitmap->getWidth(), bitmap->getHeight(), bitmap->getFormat()));
            code = tex->load(*bitmap);
            if (code == TE_Ok) {
                factory.addAtlas(std::move(atlas), std::move(tex));
                loadedFonts_[type][font].insert(style);
                return TE_Ok;
            }
        }
    }

    // not an atlas font; check if legacy rendering is available
    TextFormat2Ptr legacyFmt(nullptr, nullptr);
    if (TextFormat2_createTextFormat(legacyFmt, TextFormatParams(font, 16.f)) == TE_Ok) {
        // mark the font as legacy supported
        legacyFonts_.insert(font);
        // still return `TE_InvalidArg` as there is no associated atlas
        return TE_InvalidArg;
    }

    // font is not supported
    invalidFonts_[type][font].insert(style);
    return TE_InvalidArg;
}
...
GlyphBuffersOpts GLLabelManager::toGlyphBuffersOpts(const GLLabel &label, bool adjustFontNameIfInvalid) NOTHROWS {
    ...
    if (label.buffer_opts_.is_default_font) {
        ...
        if (defaultTextParams->bold && defaultTextParams->italic) {
            style = "bolditalic";
        } else if (defaultTextParams->bold) {
            style = "bold";
        } else if (defaultTextParams->italic) {
            style = "italic";
        }

        // set style, if necessary; small micro-optimization to avoid intern lookup
        if(!TAK::Engine::Port::String_equal(opts.style, style))
            opts.style = TAK::Engine::Port::String_intern(style);
    } else {
        if(!opts.fontName)
            opts.fontName = TAK::Engine::Port::String_intern(defaultTextParams->fontName);
        if(!opts.style)
            opts.style = TAK::Engine::Port::String_intern("regular");
        if (adjustFontNameIfInvalid && hasFont(invalidFonts_, opts.fontName, opts.style, "msdf"))
            opts.fontName = TAK::Engine::Port::String_intern(defaultTextParams->fontName);
    }
...
bool GLLabelManager::shouldUseFallbackMethod(GLLabel& label) NOTHROWS {
    auto factory = defaultGlyphBatchFactory();
    if (!factory || factory->atlasCount() == 0)
        return true;
    if (label.hints_ & GLLabel::Surface)
        return false;
#ifdef __ANDROID__
    if ((label.hints_ & GLLabel::ScrollingText) != 0 && label.textSize.width > GLLabel::MAX_TEXT_WIDTH * GLMapRenderGlobals_getRelativeDisplayDensity())
        return true;
#endif
    const auto checkver = glyphFactoryVersion_;
    if (label.should_use_fallback_method_.check == glyphFactoryVersion_)
        return label.should_use_fallback_method_.value;
    GlyphBuffersOpts opts = toGlyphBuffersOpts(label);
    label.should_use_fallback_method_.value = !factory->canRenderAllGlyphs(opts, label.text_.c_str());
    label.should_use_fallback_method_.check = checkver;
    return label.should_use_fallback_method_.value;
}
...
void GLLabelManager::queryImpl(QueryContext &result, const GLGlobeBase::State &state, const Priority priority, std::vector<GLLabel::LabelPlacement>& label_placements) NOTHROWS
{
    ...
    for(auto &it : labels_) {
        ...
        tf.font_size = gltext->getTextFormat().getFontSize();
        tf.opts = toGlyphBuffersOpts(label);
        TextFormat2& rtf = (!glyphBatchFactory || shouldUseFallbackMethod(label)) ?
            gltext->getTextFormat() : tf;

        // NOTE: this is not strictly thread-safe, however, races across threads should always result in the same value
        GLLabel::validateTextSize(&label.textSize.width, &label.textSize.height, state, label, rtf);
        ...
        GLLabel::validateTextPlacement(placements, state, label, rtf);
        ...
        for(auto &placement : placements)
        {
            ...
            {
                if(GLLabel::place(placement, state, label, NAN, labelWidth, labelHeight, tf, label_placements, deconflictMode, didReplace))
                {
                    skipped = false;
                    if(!didReplace)
                    {
    ...
    for(auto &e : replaced_labels)
    {
        ...
        const float textWidth = rtf.getStringWidth(text);
        const float textHeight = rtf.getStringHeight(text);
        const float labelWidth = (label.hints_&GLLabel::ScrollingText) ?
        ...
        for(auto &placement : e.second) {
            bool didReplace;
            if(GLLabel::place(placement, state, label, NAN, labelWidth, labelHeight, rtf, label_placements, GLLabel::PlacementDeconflictMode::Shift, didReplace)) {
                label_placements.push_back(placement);
