...
void GLLabel::LabelPlacement::recompute(const double absoluteRotation, const float width, const float height) NOTHROWS
{
    ...
    for(int i=0; i < 4;++i)
    {
        ...
        boundingRect.shape[i].x = x + static_cast<float>(render_xyz_.x) + width2;
        boundingRect.shape[i].y = y + static_cast<float>(render_xyz_.y) + height2;
    }
...
void GLLabel::setTextFormat(const TextFormatParams* fmt) NOTHROWS {
    ...
    if (textFormatParams_) {
        if (textFormatParams_->bold && textFormatParams_->italic) {
            style = "bolditalic";
        }
        else if (textFormatParams_->bold) {
            style = "bold";
        }
        else if (textFormatParams_->italic) {
            style = "italic";
        }
    }
    buffer_opts_.style = TAK::Engine::Port::String_intern(style);
...
bool GLLabel::place(GLLabel::LabelPlacement &placement, const GLGlobeBase::State& view, 
    TextFormat2& text_format, const std::vector<LabelPlacement>& label_rects, const PlacementDeconflictMode deconflictMode, bool& rePlaced) NOTHROWS {

    return place(placement, view, *this, projected_size_, labelSize.width, labelSize.height, text_format, label_rects, deconflictMode, rePlaced);
}

bool GLLabel::place(GLLabel::LabelPlacement &placement, const GLGlobeBase::State& view, 
    const GLLabel &label, const double projectedSize, const float labelWidth, const float labelHeight, TextFormat2& text_format, const std::vector<LabelPlacement>& label_rects, const PlacementDeconflictMode deconflictMode, bool& rePlaced) NOTHROWS {
    ...
    {
        placement.recompute(view.drawRotation, labelWidth, labelHeight);
    }
...
void GLLabel::validateImpl(const GLGlobeBase::State& view, TextFormat2& text_format) NOTHROWS
{
    ...
    validateTextSize(&textSize.width, &textSize.height, view, *this, text_format);
    ...
    validateTextPlacement(transformed_anchor_, view, *this, text_format);

    mark_dirty_ = false;
    draw_version_ = view.drawVersion;
}

void GLLabel::validateTextSize(float *textWidth, float *textHeight, const TAK::Engine::Renderer::Core::GLGlobeBase::State& view, const GLLabel &label, TextFormat2& text_format) NOTHROWS
{
    if(label.textSize.width*label.textSize.height) {
        *textWidth = label.textSize.width;
        *textHeight = label.textSize.height;
    } else if (!label.text_.empty()) {
        const char* text = label.text_.c_str();
        *textWidth = text_format.getStringWidth(text);
        *textHeight = text_format.getStringHeight(text);
    } else {
        *textWidth = 0.f;
        *textHeight = 0.f;
    }
}
void GLLabel::validateTextPlacement(std::vector<LabelPlacement> &placements, const TAK::Engine::Renderer::Core::GLGlobeBase::State& view, const GLLabel &label, TextFormat2& text_format) NOTHROWS
{
...
void GLLabel::validateLineStringTextPlacement(std::vector<LabelPlacement> &placements, const TAK::Engine::Renderer::Core::GLGlobeBase::State& view, const GLLabel &label, const TAK::Engine::Feature::AltitudeMode altitudeMode, const TAK::Engine::Feature::LineString2 &lineString) NOTHROWS
{
    ...
    if(altitudeMode == Feature::TEAM_ClampToGround && numPoints == 2u) {
        ...
        if(std::max(labelSize.width, labelSize.height) <= axislen) {
            textLoc = closestSurfacePointToFocus(view, (float)(axisa.x+dx*segmentPositionWeight), (float)(axisa.y+dy*segmentPositionWeight), sp, ep);
