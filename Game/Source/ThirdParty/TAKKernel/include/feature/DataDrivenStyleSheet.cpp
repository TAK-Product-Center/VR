...
namespace
{
    ...
    bool varsubst(std::string &value, const char* txt, const atakmap::util::AttributeSet& attrs, const bool allowDupeSubst) NOTHROWS;
    void flatten(std::vector<std::shared_ptr<atakmap::feature::Style>> &value, const int lod, const std::shared_ptr<const atakmap::feature::Style> &s, const atakmap::util::AttributeSet &attrs) NOTHROWS;
...
void DataDrivenStyleSheet::push(const char *layer, DataDrivenStyleSheet::StyleFilterBuilder& f_, StylePtr_const&& s) NOTHROWS
{
    ...
    if(f_.build(f)  == TE_Ok) {
        ...
        static_cast<LayerStyleImpl &>(*style).push(f, std::move(s));
    }
}
        ...
        return getStyle(value, lod, tegc, [&attrs](Attribute *attr, const char* key, const int schemaId)
    {
        ...
        for(std::size_t i = 0u; i < styleFilters.size(); i++)
        {
            ...
            if (filterMatches(filter, tegc, attrs)) {
                ...
                if (!s)
                    continue;
                flatten(xstyles, lod, std::const_pointer_cast<atakmap::feature::Style>(s), attrs);
            }
        }
#if 0
        for(auto &s : xstyles) {
...
//                            }
    ...
    bool varsubst(std::string &value, const char *txt, const atakmap::util::AttributeSet &attrs,
                  const bool allowDupeSubst) NOTHROWS {
    ...
    void flatten(std::vector<std::shared_ptr<atakmap::feature::Style>> &value, const int lod_, const std::shared_ptr<const atakmap::feature::Style> &s, const atakmap::util::AttributeSet &attrs) NOTHROWS
    {
        switch(s->getClass()) {
            case atakmap::feature::TESC_LevelOfDetailStyle :
            {
                ...
                if(lod_ == -1) {
                    ...
                    for (std::size_t i = 0u; i < numLods; i++) {
                        ...
                        if (i == (numLods - 1u))
                            lod--;
                        flatten(children, -1, impl.getStyle(lod), attrs);
                    }
                    ...
                    if(child)
                        flatten(value, lod_, child, attrs);
                }
                break;
            }
