...
#ifdef __LINUX__
// Current builds of takkernel on Linux use gcc 4.8, but regex support isn't fully implemented until gcc 4.9 (see ENGINE-709).
TAKErr FileProtocolHandler::handleURI(DataInput2Ptr &ctx, const char * uri) NOTHROWS
{
    ...
    if (target.find("file://") == 0) target.erase(0, 7);
    if (target.find("file:/") == 0) target.erase(0, 6);

    return IO_openFile(ctx, target.c_str());
}
...
TAKErr ZipProtocolHandler::handleURI(DataInput2Ptr &ctx, const char * uri) NOTHROWS
{
    ...
    std::string::size_type path_delim = target.find("://");
    if (path_delim == std::string::npos) {
        Logger_log(TELL_Warning, "Malformed zip URI: %s", uri);
        return TE_InvalidArg;
    }
    std::string scheme = target.substr(0, path_delim);
    path_delim += 3;

    std::string::size_type entry_delim = target.find("!", path_delim);
    if (entry_delim == std::string::npos) {
        Logger_log(TELL_Warning, "Malformed zip URI: %s", uri);
        return TE_InvalidArg;
    }
    std::string archive = target.substr(path_delim, entry_delim - path_delim);
    entry_delim += 1;
    std::string entry = target.substr(entry_delim);
    ...
    if (!!zipCompare && !!arcCompare) {
        Logger_log(TELL_Warning, "Incorrect URI scheme, \"%s,\" in ZipProtocolHandler; full URI: %s", scheme.c_str(), uri);
        return TE_InvalidArg;
    }

    code = normalizePath(archive, archive, true);
    TE_CHECKRETURN_CODE(code);
    ...
    code = normalizePath(entry, entry, false);
    TE_CHECKRETURN_CODE(code);
