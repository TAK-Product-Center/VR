...
public:
	...
	*/
	...
	UFUNCTION(BlueprintPure)
	TSet<FString> GetGroupRoles() const;

    /* returns true if inRole is a valid role */
	UFUNCTION(BlueprintPure)
	bool IsGroupRole(const FString& inRole) const;
    ...
    UFUNCTION(BlueprintCallable)
    void QueryServersForDataPackages();
...
protected:
	...
	FString SanitizeRawCOTAttributes(FString inCot);
