    ...
    public atakmap::commoncommo::CoTSendFailureListener
{
    virtual void cotMessageReceived(const char* cotMessage, const char* rxIfaceEndpointId)
    {
        ...
        UVH_COTManager::Get()->HandleCotMessageReceived(FString(cotMessage), FString(rxIfaceEndpointId));
    }
    ...
    virtual void contactAdded(const atakmap::commoncommo::ContactUID* c)
    {
        ...
        UVH_COTManager::Get()->HandleContactUpdated(FName(*uidString), true);
    }

    virtual void contactRemoved(const atakmap::commoncommo::ContactUID* c)
    {
        FString uidString = vh_cot::ConvertFromCommoString(BytesToString(c->contactUID, c->contactUIDLen));
        UVH_COTManager::Get()->HandleContactUpdated(FName(*uidString), false);
    }
    ...
    virtual atakmap::commoncommo::CoTPointData getCurrentPoint()
    {
        ...
        //return UVH_COTManager::Get()->GetCurrentPoint();
    ...
    {
        UVH_COTManager::Get()->HandleInterfaceStatusUpdate(iface, true);
    }

    virtual void interfaceDown(atakmap::commoncommo::NetInterface* iface)
    {
        UVH_COTManager::Get()->HandleInterfaceStatusUpdate(iface, false);
    }
    ...
    {
        UVH_COTManager::Get()->HandleInterfaceError(iface, err);
    }
...
{
	const FString& cotMessageSanitized = SanitizeRawCOTAttributes(cotMessage);

	AsyncTask(ENamedThreads::GameThread, [this, cotMessageSanitized, rxIfaceEndpointId]()
	{
		...
		{
			UWorld* world = cotManager->GetWorld();
			if (IsValid(world) && world->IsValidLowLevelFast())
			{
			...
			if (IsValid(cotManager) && cotManager->IsValidLowLevelFast() && IsValid(cotManager->GetClass()) && cotManager->GetClass()->IsValidLowLevelFast() && cotManager->bEnabled)
			{
				...
				//UE_LOG(VH_COT, Verbose, TEXT("-- received cot: %s"), *cotMessageSanitized);
...
void UVH_COTManager::PopulateDataSyncsFromTAKServer()
{
	...
	if (GetWorld()->GetNetMode() != NM_Client)
	{
		for (UVH_COTConnectionInfo* connectionInfo : ConnectionInfos)
		{
			if (connectionInfo->bConnected)
			{
				FString url = FString::Printf(TEXT("https://%s:8443/Marti/api/missions"), *connectionInfo->Host);
...
TArray<UVH_DataSync*> UVH_COTManager::GetSortedDataSyncs(EDataSyncSort sortType, const FString& filter) const
{
	...
	if (!filter.IsEmpty())
	{
		...
		dataSyncs.GenerateKeyArray(names);
...
TSet<FString> UVH_COTManager::GetGroupRoles() const
{
	return GroupRoles;
}

bool UVH_COTManager::IsGroupRole(const FString& inRole) const
{
	return GroupRoles.Contains(inRole);
}
...
void UVH_COTManager::QueryServersForDataPackages()
{
...
FString UVH_COTManager::SanitizeRawCOTAttributes(FString inCot)
{
	FString newCOT = inCot;
	
	int32 index = 1;
	bool inQuotes = false;

	while (index < newCOT.Len())
	{
		if (newCOT[index] == '"' && !(newCOT[index-1] == '\\'))
		{
			inQuotes = !inQuotes;
		}
		...
		if (inQuotes)
		{
			int32 nextQuotes = newCOT.Find("\"", ESearchCase::CaseSensitive, ESearchDir::FromStart, index + 1);
			FString prevSubstring = newCOT.Left(index + 1);
			FString quotesSubstring = newCOT.Mid(index + 1, nextQuotes - (index + 1));
			FString postSubstring = newCOT.RightChop(nextQuotes);

			// make sure we don't override any properly escaped characters
			quotesSubstring.ReplaceInline(TEXT("&quot;"), TEXT("\""),  ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline(TEXT("&apos;"), TEXT("'"),  ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline(TEXT("&lt;"), TEXT("<"),  ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline(TEXT("&gt;"), TEXT(">"),  ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline(TEXT("&amp;"), TEXT("&"),  ESearchCase::CaseSensitive);

			// escape escapable characters
			quotesSubstring.ReplaceInline( TEXT("&"), TEXT("&amp;"), ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline( TEXT("\""), TEXT("&quot;"), ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline( TEXT("'"), TEXT("&apos;"), ESearchCase::CaseSensitive);
			quotesSubstring.ReplaceInline( TEXT("<"), TEXT("&lt;"), ESearchCase::CaseSensitive);
			...
			newCOT = prevSubstring + quotesSubstring + postSubstring;

			index += quotesSubstring.Len() + 2;
			inQuotes = !inQuotes;
		}
		else
		{
...
bool UVH_COTManager::HandleGeochatEvent(UVH_COTEvent* cotEvent)
{
	if (cotEvent->GetAttribute(TEXT("type")).Equals(TEXT("b-t-f")))
	{
		...
		if (UVH_COTEvent** cotEventPtr = Contacts.Find(uid0))
		{
			...
			chatItem->Message = cotEvent->GetNodeContent(TEXT("detail\tremarks\tmessage"));
...
void UVH_COTManager::InitializeRoles()
{
	GroupRoles =
	{
		FString("Team Member"),
...
void UVH_COTManager::AddGetCOTBoundingBoxDelegate()
{
	...
	boundingBoxDelegate.BindLambda([](UVH_COTEvent* cotEvent, FGeodeticCoord2D& northWest, FGeodeticCoord2D& southEast)
	{
		...
		{
			// circle
			double maxRadiusMeters = 180;

			// get the greatest maximum radius of any ellipse
			...
			northWest = UVH_GlobeMathStatics::CalculateSurfaceDestination(geodeticCoordinate, 315.f * maxRadiusMeters*FMath::Sqrt(2.f), maxRadiusMeters);
			southEast = UVH_GlobeMathStatics::CalculateSurfaceDestination(geodeticCoordinate, 135.f * maxRadiusMeters*FMath::Sqrt(2.f), maxRadiusMeters);
