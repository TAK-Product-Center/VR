...
// Game Includes
#include "Game_Subsystem_Keyboard_Settings.h"
...
UGame_Subsystem_Keyboard* UGame_Subsystem_Keyboard::Get()
{
	if (UWorld* world = UDeveloper_Statics::GetVHWorld())
	{
		return world->GetSubsystem<UGame_Subsystem_Keyboard>();
	}
...
void UGame_Subsystem_Keyboard::Initialize(FSubsystemCollectionBase& collection)
{
	...
	if (const UGame_Subsystem_Keyboard_Settings* settings = GetDefault<UGame_Subsystem_Keyboard_Settings>())
	{
...
UClass* UGame_Subsystem_Keyboard::GetKeyboardClass() const
{
	if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
	{
		...
		{
			FString keyboard = gameSettings->GetSettingString(TEXT("Keyboard"));

			if (TSubclassOf<AActor>* classPtr = subsystem->KeyboardClassMap.Find(keyboard))
			{
				return *classPtr;
			}
		}
...
void UGame_Subsystem_Keyboard::UpdateSettings(const UGame_Subsystem_Keyboard_Settings* settings)
{
	KeyboardClassMap = settings->KeyboardClassMap;
}
				...
				if (newFocusedWidget.IsValid())
				{
					if (KeyboardWidgets.Contains(newFocusedWidget->GetTypeAsString()))
					{
						...
						if (hardwareInterfaceMode == EHardwareInterfaceMode::VREmulation)
						{
							...
							UDeveloper_Statics::PressAndReleaseKey(EKeys::End);
						}
						else if (hardwareInterfaceMode == EHardwareInterfaceMode::VR)
						{
							UGame_Statics::GetVHPlayerController()->PlayerInput->FlushPressedKeys();
							...
							if (UKeyboard_ContainerComponent* containerComponent = keyboardContainer->GetComponentByClass<UKeyboard_ContainerComponent>())
							{
								...
								slateApplication.SetAllUserFocusToGameViewport();
							}
						}
					}
				}
