...
void UGame_UserWidget_DetailSection_EditPoints::PopulatePolylineEditPoints()
{
	...
	bool bClosed = UVH_COTStatics::IsPolylineClosed(COTEvent);
	
	for (int32 i = 0; i < links.Num(); i++)
	{
		// don't show last coordinate if polyline is closed
		if (!bClosed || i != links.Num() - 1)
		{
			FGeodeticCoord3D linkCoordinate = UVH_COTStatics::LinkToCoordinate(links[i]);
...
void UGame_UserWidget_DetailSection_EditPoints::PopulateRangeBearingEditPoints()
{
	...
	{
		arrowCoord3D.Altitude = baseHeight + FMath::Tan(inclinationRadians) * range;
	}
...
void UGame_UserWidget_DetailSection_EditPoints::SetRangeBearingLocation(int32 index, const FGeodeticCoord3D& coordinate)
{
	FGeodeticCoord3D baseCoordinate;
	FGeodeticCoord3D arrowCoordinate;

	if (index == 0)
	{
		// base

		double range = COTEvent->GetAttribute<double>(TEXT("detail\trange\tvalue"));  // ground distance, not slant distance
		double bearingDegrees = COTEvent->GetAttribute<double>(TEXT("detail\tbearing\tvalue"));
		double inclinationRadians = FMath::DegreesToRadians(COTEvent->GetAttribute<double>(TEXT("detail\tinclination\tvalue")));

		baseCoordinate = COTEvent->GetPoint();
		...
		arrowCoordinate = FGeodeticCoord3D(arrowSurfaceCoordinate, baseCoordinate.Altitude + (FMath::Tan(inclinationRadians) * range));
	...
	double range = UVH_GlobeMathStatics::CalculateSurfaceDistance(baseCoordinate.ToGeodeticCoord2D(), arrowCoordinate.ToGeodeticCoord2D());
	COTEvent->SetAttribute<double>(TEXT("detail\trange\tvalue"), range);

	COTEvent->SetAttribute<double>(TEXT("detail\tbearing\tvalue"), UVH_GlobeMathStatics::CalculateSurfaceBearing(baseCoordinate.ToGeodeticCoord2D(), arrowCoordinate.ToGeodeticCoord2D()));
	...
	else if (FMath::IsNearlyEqual(arrowCoordinate.Altitude, 9999999.0))
	{
		...
		{
			double arrowHAE;
			if (tileManager->GetHAE(arrowCoordinate.ToGeodeticCoord2D(), arrowHAE))
			{
				arrowCoordinate.Altitude = arrowHAE;
		...
		if (!bFoundHeight)
		{
			// couldn't find height, set arrow to same height as base
			arrowCoordinate.Altitude = baseCoordinate.Altitude;
		}
	}
