			...
			if (tileManager->GetTerrainGrid(GridX, GridY, RadiusMeters, viewshedCenter, averageGridVertices))
			{
				...
				if (IsValid(UDeveloper_Statics::GetVHWorld()))
				{
...

					{
						...
						AVH_GlobeReferenceActor* globeReference = nullptr;
						float unitsPerMeter = 100;
						if (IsValid(virtualObjective))
						{
							unitsPerMeter = virtualObjective->GetUnrealUnitsPerMeter();
							globeReference = virtualObjective->GetGlobeReference();
						}

						ParallelFor(redStarts.Num(), [this, &waitCriticalSection, &addCriticalSection, queryParams, totalLocations, redEnds, redStarts, globeReference, unitsPerMeter](int32 i)
						{
							ParallelFor(PointsPerBeam, [this, &waitCriticalSection, &addCriticalSection, queryParams, i, totalLocations, redEnds, redStarts, globeReference, unitsPerMeter](int32 j)
							{
								double lerpAmount = (double)j / PointsPerBeam;
								lerpAmount += FMath::RandRange(-0.01, 0.01);
								...
								if (bTraceGround)
								{
									if (IsValid(globeReference))
									{
										FGeodeticCoord3D coordinate = globeReference->WorldToGeodeticDouble(worldLocation);

										double elevationMeters = globeReference->GetTerrainElevationDouble(coordinate.ToGeodeticCoord2D());

										FTransform surfaceTransform = UGame_Statics::GetWorldTransformOverride(worldLocation);
										worldLocation = surfaceTransform.TransformPosition(FVector(0, 0, (elevationMeters - coordinate.Altitude + 5) * unitsPerMeter));
									}
								}
