...
void AGame_Actor_KmlPlacemark::SetActorHiddenInGame(bool bNewHidden)
{
			...
			if (AGame_Actor_KmlPoint* pointActor = GetWorld()->SpawnActor<AGame_Actor_KmlPoint>(PointActorClass, GetActorTransform(), spawnParams))
			{
				...
				if (iconHRef.StartsWith(TEXT("http")))
				{
					if (iconHRef.StartsWith(TEXT("http://maps.google.com")))
					{
						...
						scaledDimensions = FVector2D(iconTexture->GetSizeX(), iconTexture->GetSizeY());
						originalDimensions = scaledDimensions;
					}
				}
				else if (iconHRef.StartsWith(TEXT("data")))
				{
					TArray<FString> stringData;
					iconHRef.ParseIntoArray(stringData, TEXT(","));

					if (stringData.Num() > 1)
					{
						// assumes encoded as 64bit string
						TArray<uint8> data;
						if (FBase64::Decode(stringData[1], data))
						{
							std::vector<unsigned char> dataVector(data.GetData(), data.GetData() + data.Num());
							cv::Mat mat = cv::imdecode(dataVector, cv::IMREAD_UNCHANGED);
						
							if (!mat.empty())
							{
								bool bHandled = false;

								// resize texture to nearest square with max of 4096
								uint32 widthPowerOf2 = FMath::RoundUpToPowerOfTwo(mat.cols);
								uint32 heightPowerOf2 = FMath::RoundUpToPowerOfTwo(mat.rows);

								uint32 powerOf2 = FMath::Max<uint32>(widthPowerOf2, heightPowerOf2);
								powerOf2 = FMath::Min<uint32>(powerOf2, 4096);

								originalDimensions = FVector2D(mat.cols, mat.rows);
								scaledDimensions = FVector2D(powerOf2, powerOf2);

								cv::Mat resizedMat(powerOf2, powerOf2, mat.type(), 0.0);
								
								//mat.copyTo(resizedMat(cv::Rect(0, 0, mat.cols, mat.rows)));


								cv::Mat convertMat;
								try
								{
									cv::resize(mat, resizedMat, cv::Size(powerOf2, powerOf2));

									if (resizedMat.type() == CV_8UC1)
									{
										cv::cvtColor(resizedMat, convertMat, cv::COLOR_GRAY2BGRA);
										bHandled = true;
									}
									else if (resizedMat.type() == CV_8UC3)
									{
										cvtColor(resizedMat, convertMat, cv::COLOR_BGR2BGRA);
										bHandled = true;
									}
									else if (resizedMat.type() == CV_8UC4)
									{
										convertMat = resizedMat.clone();
										bHandled = true;
									}
								}
								catch (cv::Exception& e)
								{
									...
									UDeveloper_Statics::PrintMessageError(FString::Printf(TEXT("-- AGame_Actor_KmlPlacemark::LoadKmlPoint | opencv error: %s"), ANSI_TO_TCHAR(err_msg)));
								}

								if (bHandled)
								{
									TArray<uint8> pixels;
									pixels.AddZeroed(convertMat.total() * convertMat.elemSize());
									FMemory::Memcpy(pixels.GetData(), convertMat.data, pixels.Num());

									iconTexture = UDeveloper_Statics::CreateTextureFromPixels(convertMat.cols, convertMat.rows, pixels);
								}
								else
								{
									UE_LOG(Game, Warning, TEXT("-- AGame_Actor_KmlPlacemark::LoadKmlPoint | unhandled mat type | %d"), mat.type());
								}
							}
						}
					}
				}
				else if (!KmlInfo->FullPathToFile.IsEmpty())
				{
				...
				{
					pointActor->ReceiveSetTexture(iconTexture, originalDimensions.X / originalDimensions.Y);
				}
