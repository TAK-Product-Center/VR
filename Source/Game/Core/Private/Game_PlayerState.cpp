...
void AGame_PlayerState::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AGame_PlayerState, PlayerTransformOverride);
...
void AGame_PlayerState::Server_StopPresenting_Implementation()
{
	if (AGame_GameState* gameState = UGame_Statics::GetVHGameState())
	{
		...
		gameState->SetIsPresentingPlayerLocked(false);
	}
}

void AGame_PlayerState::Server_SetPresentingLocked_Implementation(bool isLocked)
{
	if (AGame_GameState* gameState = UGame_Statics::GetVHGameState())
	{
		gameState->SetIsPresentingPlayerLocked(isLocked);
	}
}

void AGame_PlayerState::Server_SetPlayerTransformOverride_Implementation(const FTransform& transformOverride)
{
	PlayerTransformOverride = transformOverride;

	OnPlayerTransformOverrideChanged.Broadcast(transformOverride);
}
...
void AGame_PlayerState::OnRep_PlayerTransformOverride()
{
	OnPlayerTransformOverrideChanged.Broadcast(PlayerTransformOverride);
}
...
bool AGame_PlayerState::IsInTopdownMode() const
{
	return !bIsInTOC && PlayerTransformOverride.Equals(FTransform::Identity);
}
