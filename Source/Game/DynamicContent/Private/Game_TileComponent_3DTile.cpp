...
bool UGame_TileComponent_3DTile::NodeIntersects(UGame_3DTileNode* node)
{
	...
	if (!bInitialPass)
	{
		// max distance between user and edge of tile
		FGeodeticCoord2D nw = GetNorthWestBound();
		FGeodeticCoord2D se = GetSouthEastBound();
		
		FGeodeticCoord2D ne = nw;
		ne.Longitude = se.Longitude;

		FGeodeticCoord2D sw = nw;
		sw.Latitude = se.Latitude;

		FGeodeticCoord2D playerCoordinate = GetPlayerGeoLocation().ToGeodeticCoord2D();

		maxDistanceMeters = UVH_GlobeMathStatics::CalculateSurfaceDistance(playerCoordinate, nw);
		maxDistanceMeters = FMath::Max<double>(maxDistanceMeters, UVH_GlobeMathStatics::CalculateSurfaceDistance(playerCoordinate, ne));
		maxDistanceMeters = FMath::Max<double>(maxDistanceMeters, UVH_GlobeMathStatics::CalculateSurfaceDistance(playerCoordinate, se));
		maxDistanceMeters = FMath::Max<double>(maxDistanceMeters, UVH_GlobeMathStatics::CalculateSurfaceDistance(playerCoordinate, sw));
	}
