...
void UGame_MatComponent_CesiumIon::ExecuteHelperThreadLogic()
{
	...
	if (GetComponentStatus() == EComponentStatus::Initialized)
	{
		...
		if (UDeveloper_PersistentSettings* gameSettings = UGame_Subsystem_Core::GetGameSettings())
		{
			...
			GammaCorrection = gameSettings->GetSettingDouble(TEXT("cesium_imagery.GammaCorrection"));
		}
		...
		if (!accessToken.IsEmpty() && !assetID.IsEmpty())
		{
			...
			request->OnProcessRequestComplete().BindLambda([this, assetID](FHttpRequestPtr request, FHttpResponsePtr response, bool bSucceeded)
			{
				...
				if (FJsonSerializer::Deserialize(jsonReader, jsonObject))
				{
					...
					if (jsonObject->TryGetStringField(TEXT("externalType"), externalType))
					{
						if (externalType.Equals(TEXT("bing"), ESearchCase::IgnoreCase))
						{
							...
							if (jsonObject->TryGetObjectField(TEXT("options"), optionsObjectPtr))
							{
								...
								FString mapStyle;
								optionsObject->TryGetStringField(TEXT("mapStyle"), mapStyle);

								FString key;
								optionsObject->TryGetStringField(TEXT("key"), key);

								SourceName = FString::Printf(TEXT("cesiumImagery%s"), *assetID);
								SourceType = TEXT("png");
								Projection = EProjection::Mercator;

								HandleExternalBing(mapStyle, key);
						...
						{
							// unhandled external type 
							UE_LOG(Game, Warning, TEXT("-- UGame_MatComponent_CesiumIon::ExecuteHelperThreadLogic | unhandled external type | aborting"));

							// make a crimson mat to show error
				...
				try
				{
					if (tileMat.type() != allTiles.type())
					{
						...
						if (tileMat.type() == CV_8UC1)
						{
							...
							AddGammaCorrection(convertMat, gammaCorrected, GammaCorrection);
...
void UGame_MatComponent_CesiumIon::HandleExternalBing(const FString& imagerySet, const FString& bingKey)
{
	FString url = FString::Printf(TEXT("https://dev.virtualearth.net/REST/v1/Imagery/Metadata/%s?key=%s"), *imagerySet, *bingKey);

	TSharedRef<IHttpRequest, ESPMode::ThreadSafe> metadataRequest = (&FHttpModule::Get())->CreateRequest();
	metadataRequest->SetURL(url);
	metadataRequest->SetVerb(TEXT("GET"));

	metadataRequest->OnProcessRequestComplete().BindLambda([this, imagerySet, bingKey](FHttpRequestPtr request, FHttpResponsePtr response, bool bSucceeded)
	{
		if (response.IsValid())
		{
			TSharedRef<TJsonReader<>> jsonReader = TJsonReaderFactory<>::Create(*response->GetContentAsString());
			...
			if (FJsonSerializer::Deserialize(jsonReader, jsonObject))
			{
				const TArray<TSharedPtr<FJsonValue>>* resourceSetsArrayPtr = nullptr;
				if (jsonObject->TryGetArrayField(TEXT("resourceSets"), resourceSetsArrayPtr))
				{
					TArray<TSharedPtr<FJsonValue>> resourceSets = *resourceSetsArrayPtr;
					for (int32 i = 0; i < resourceSets.Num(); i++)
					{
						TSharedPtr<FJsonObject> resourceSetObject = resourceSets[i]->AsObject();
						
						const TArray<TSharedPtr<FJsonValue>>* resourcesArrayPtr = nullptr;
						if (resourceSetObject->TryGetArrayField(TEXT("resources"), resourcesArrayPtr))
						{
							TArray<TSharedPtr<FJsonValue>> resources = *resourcesArrayPtr;
							for (int32 j = 0; j < resources.Num(); j++)
							{
								TSharedPtr<FJsonObject> resourceObject = resources[j]->AsObject();

								TArray<FString> serverParts;
								const TArray<TSharedPtr<FJsonValue>>* urlSubdomainsArrayPtr = nullptr;
								if (resourceObject->TryGetArrayField(TEXT("imageUrlSubdomains"), urlSubdomainsArrayPtr))
								{
									TArray<TSharedPtr<FJsonValue>> urlSubdomains = *urlSubdomainsArrayPtr;
									for (int32 k = 0; k < urlSubdomains.Num(); k++)
									{
										serverParts.Add(urlSubdomains[k]->AsString());
									}
								}

								FString imageURL;
								if (resourceObject->TryGetStringField(TEXT("imageURL"), imageURL))
								{
									// determine tiles to pull
									TileUtilities::GetTileFromGeodetic(GetNorthWestBound(), GetZoomLevel(), NorthWestTile);
									TileUtilities::GetTileFromGeodetic(GetSouthEastBound(), GetZoomLevel(), SouthEastTile);
									...
									for (int32 y = FMath::Min<int32>(NorthWestTile.Value, SouthEastTile.Value); y <= FMath::Max<int32>(NorthWestTile.Value, SouthEastTile.Value); y++)
									{
										for (int32 x = FMath::Min<int32>(NorthWestTile.Key, SouthEastTile.Key); x <= FMath::Max<int32>(NorthWestTile.Key, SouthEastTile.Key); x++)
										{
											...
											if (!TileToMatData.Contains(tileNumber))
											{
												...
												requestURL.ReplaceInline(TEXT("{quadkey}"), *TileUtilities::TileToQuadKey(GetZoomLevel(), tileNumber.Key, tileNumber.Value), ESearchCase::IgnoreCase);

												if (serverParts.Num() > 0)
												{
													requestURL.ReplaceInline(TEXT("{subdomain}"), *serverParts[FMath::RandHelper(serverParts.Num())]);
												}

												FHttpModule* http = &FHttpModule::Get();

												TSharedRef<IHttpRequest, ESPMode::ThreadSafe> imageryRequest = http->CreateRequest();
												imageryRequest->SetHeader(TEXT("User-Agent"), TEXT("TAK"));
												imageryRequest->SetURL(requestURL);
												imageryRequest->AppendToHeader(TEXT("email"), FString::Printf(TEXT("%d.%d.%d.%s.%s"), tileNumber.Key, tileNumber.Value, GetZoomLevel(), *SourceName, *SourceType));
												imageryRequest->SetVerb(TEXT("GET"));

												tileManager->AddHTTPRequest(tileIdentifier, this, imageryRequest);
		...
		{
			UE_LOG(Game, Warning, TEXT("-- UGame_MatComponent_CesiumIon::HandleExternalBing | invalid response | aborting"));
		}
	});

	metadataRequest->ProcessRequest();
}

void UGame_MatComponent_CesiumIon::AddGammaCorrection(cv::Mat& src, cv::Mat& dst, float fGamma)
{
