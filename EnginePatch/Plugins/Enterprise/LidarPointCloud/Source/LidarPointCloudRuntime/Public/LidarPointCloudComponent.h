...
public:
	...
	{
		// WITH_VH
		if (PointCloud != nullptr && PointCloud->IsValidLowLevelFast())
		{
			return PointCloud->LineTraceSingle(Ray.TransformBy(GetComponentTransform().Inverse()), Radius, bVisibleOnly);
		}
		else
		{
			return nullptr;
		...
		//return PointCloud ? PointCloud->LineTraceSingle(Ray.TransformBy(GetComponentTransform().Inverse()), Radius, bVisibleOnly) : nullptr;  // WITH_VH orig
	}
