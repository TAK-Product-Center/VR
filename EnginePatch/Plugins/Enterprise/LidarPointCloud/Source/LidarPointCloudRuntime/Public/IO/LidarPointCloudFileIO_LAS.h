...
struct FLidarPointCloudImportSettings_LAS : public FLidarPointCloudImportSettings
{
	...
	virtual TSharedPtr<FLidarPointCloudImportSettings> Clone(const FString& NewFilename = "") override
	{
		...
		NewSettings->ImportScale = ImportScale;  // WITH_VH
		return NewSettings;
	}
};
...
private:
	//bool HandleImportLAS(const FString& Filename, FLidarPointCloudImportResults& OutImportResults);  // WITH_VH
	bool HandleImportLAS(const FString& Filename, TSharedPtr<FLidarPointCloudImportSettings> ImportSettings, FLidarPointCloudImportResults& OutImportResults);  // WITH_VH
	bool HandleExportLAS(const FString& Filename, ULidarPointCloud* PointCloud);

#if LASZIPSUPPORTED
	//bool HandleImportLAZ(const FString& Filename, FLidarPointCloudImportResults& OutImportResults);  // WITH_VH
	bool HandleImportLAZ(const FString& Filename, TSharedPtr<FLidarPointCloudImportSettings> ImportSettings, FLidarPointCloudImportResults& OutImportResults);  // WITH_VH
