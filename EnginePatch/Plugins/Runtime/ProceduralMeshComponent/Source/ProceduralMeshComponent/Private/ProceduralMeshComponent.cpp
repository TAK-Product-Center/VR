...
: Super(ObjectInitializer)
{
	...
	// WITH_VH
	NoCollisionWithinBoxTransform = FTransform::Identity;
	// end WITH_VH
}
	...
	if(SectionIndex < ProcMeshSections.Num())
	{
		...
		if (bSameVertexCount)
		{
			...
			if (Section.bEnableCollision)
			{
				...
				//for (const FProcMeshSection& CollisionSection : ProcMeshSections)  // WITH_VH
				for (FProcMeshSection& CollisionSection : ProcMeshSections)  // WITH_VH
				{
					...
					if (CollisionSection.bEnableCollision)
					{
						...
						{
							// WITH_VH
							if (!NoCollisionWithinBoxTransform.Equals(FTransform::Identity))
							{
								FBox unitBox;
								unitBox += FVector(-1, -1, -1);
								unitBox += FVector(1, 1, 1);

								// transform vertex into box's local space
								FVector vertexInBoxLocal = NoCollisionWithinBoxTransform.InverseTransformPosition(CollisionSection.ProcVertexBuffer[VertIdx].Position);

								if (bCutAllCollision || unitBox.IsInsideOrOnXY(vertexInBoxLocal))
								{
									// remove any associated triangles
									for (int32 i = 0; i < CollisionSection.ProcIndexBuffer.Num(); i += 3)
									{
										if (CollisionSection.ProcIndexBuffer[i] == VertIdx ||
											CollisionSection.ProcIndexBuffer[i + 1] == VertIdx ||
											CollisionSection.ProcIndexBuffer[i + 2] == VertIdx)
										{
											CollisionSection.ProcIndexBuffer.RemoveAt(i, 3);
										}
									}
...
bool UProceduralMeshComponent::GetPhysicsTriMeshData(struct FTriMeshCollisionData* CollisionData, bool InUseAllTriData)
{
	...
	for (int32 SectionIdx = 0; SectionIdx < ProcMeshSections.Num(); SectionIdx++)
	{
		...
		if (Section.bEnableCollision)
		{
			TSet<int32> removedVerts;  // WITH_VH
		
			// Copy vert data
			for (int32 VertIdx = 0; VertIdx < Section.ProcVertexBuffer.Num(); VertIdx++)
			{
				// WITH_VH
				// only check if we're not cutting all collision and we have a valid cut
				if (!bCutAllCollision && !NoCollisionWithinBoxTransform.Equals(FTransform::Identity))
				{
					FBox unitBox;
					unitBox += FVector(-1, -1, -1);
					unitBox += FVector(1, 1, 1);
					...
					// transform vertex into box's local space
					FVector vertexInBoxLocal = NoCollisionWithinBoxTransform.InverseTransformPosition(Section.ProcVertexBuffer[VertIdx].Position);

					if (unitBox.IsInsideOrOnXY(vertexInBoxLocal))
					{
						removedVerts.Add(VertIdx);
					}
				}
				// end WITH_VH
			...
			for (int32 TriIdx = 0; TriIdx < NumTriangles; TriIdx++)
			{
				...
				if (removedVerts.Num() > 0)
				{
					if (removedVerts.Contains(Triangle.v0 - VertexBase) || removedVerts.Contains(Triangle.v1 - VertexBase) || removedVerts.Contains(Triangle.v2 - VertexBase))
					{
						continue;
					}
				}
...
// WITH_VH
int32 UProceduralMeshComponent::GetNumCollisionsQueued() const
{
	return AsyncBodySetupQueue.Num();
}

void UProceduralMeshComponent::UpdateCollisionPublic()
{
	UpdateCollision();
}
// end WITH_VH
