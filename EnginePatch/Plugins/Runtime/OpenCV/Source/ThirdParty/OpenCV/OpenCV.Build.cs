...
public class OpenCV : ModuleRules
{
	public OpenCV(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			...
			//PublicDelayLoadDLLs.Add(DLLName);  // WITH_VH
			//RuntimeDependencies.Add(Path.Combine(BinaryPath, DLLName));  // WITH_VH
			RuntimeDependencies.Add(Path.Combine("$(TargetOutputDir)", DLLName), Path.Combine(BinaryPath, DLLName), StagedFileType.NonUFS);  // WITH_VH
