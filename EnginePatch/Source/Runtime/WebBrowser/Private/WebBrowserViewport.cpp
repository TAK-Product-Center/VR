...
#include "IHeadMountedDisplay.h"  // WITH _VH
#include "IXRTrackingSystem.h"  // WITH_Vh
#include "Engine/Engine.h" // WITH_VH
...
{
	// WITH_VH
	IHeadMountedDisplay* hmd = GEngine != nullptr && GEngine->XRSystem.IsValid() ? GEngine->XRSystem->GetHMDDevice() : nullptr;
	if (hmd == nullptr || !hmd->IsHMDEnabled())
	{
		OnFocused.Broadcast(!bIsPopup);
	}
	// end WITH_VH
