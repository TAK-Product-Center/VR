...
#include "Framework/Application/SlateUser.h"  // WITH_VH
...
void SWebBrowserView::Construct(const FArguments& InArgs, const TSharedPtr<IWebBrowserWindow>& InWebBrowserWindow)
{
	...
	if (BrowserWindow.IsValid())
	{
		...
		// WITH_VH
		BrowserViewport->OnFocused.AddLambda([this](bool bFocused)
		{
			//bForceFocus = bFocused;

			if (bFocused)
			{
				if (FSlateApplication::Get().GetUserFocusedWidget(0) != BrowserWidget.ToSharedRef())
				{
					FSlateApplication::Get().SetKeyboardFocus(BrowserWidget.ToSharedRef(), EFocusCause::SetDirectly);
				}
			}
			else
			{
				...
				{
					if (User.GetFocusedWidget() == BrowserWidget.ToSharedRef())
					{
						User.ClearFocus();
					}
				}, true);
...
bool SWebBrowserView::UnhandledKeyDown(const FKeyEvent& KeyEvent)
{
	...
	// WITH_VH | prevents keyboard input from bubbling to game when entering text to web browser widget
	if (BrowserWidget->HasKeyboardFocus())
	{
		return true;
	}
	// end WITH_VH
...
// WITH_VH | forwards keyboard focus to the SWidget for browser text
