...
// WITH_VH
void FCEFBrowserHandler::OnBeforeDownload(CefRefPtr<CefBrowser> browser, CefRefPtr<CefDownloadItem> download_item, const CefString& suggested_name, CefRefPtr<CefBeforeDownloadCallback> callback)
{
	FString suggestedName = WCHAR_TO_TCHAR(suggested_name.ToWString().c_str());

	IWebBrowserSingleton* webBrowserSingleton = IWebBrowserModule::Get().GetSingleton();
	FString downloadPath(FPaths::Combine(webBrowserSingleton->ApplicationCacheDir(), TEXT("webcache"), TEXT("Downloads"), suggestedName));

	callback->Continue(TCHAR_TO_WCHAR(*downloadPath), false);
}

void FCEFBrowserHandler::OnDownloadUpdated(CefRefPtr<CefBrowser> browser, CefRefPtr<CefDownloadItem> download_item, CefRefPtr<CefDownloadItemCallback> callback)
{
}
// end WITH_VH
...
{
	//if (CefCurrentlyOn(TID_UI))  // WITH_VH
	if (CefCurrentlyOn(TID_UI) && FSlateApplication::IsInitialized())  // WITH_VH
	{
