...
// WITH_VH
bool FXmlFile::SaveToString(FString& xmlString)
{
	//FString Xml = TEXT("<?xml version=\"1.0\" encoding=\"UTF-8\"?>") LINE_TERMINATOR;
...
void FXmlFile::PreProcessInput(TArray<FString>& Input)
{
	...
	for(int32 i = 0; i < Input.Num(); ++i)
	{
		...
		{
#if 1	// WITH_VH
			// Remove and check again... for cases where more stuff follows this tag on the same line -- e.g. sometimes the entire file is just 1 line!
			Input[i].MidInline(Input[i].Find(TEXT(">")) + 1);
			i--;
#else	// WITH_VH (orig)
			Input[i] = TEXT("");
#endif	// end WITH_VH
...
void FXmlFile::Tokenize(FStringView Input, TArray<FString>& Tokens)
{
	...
	for (const TCHAR* Ptr = PtrStart; Ptr != PtrEnd; ++Ptr)
	{
		...
		else // STRING
		{
			...
			// WITH_VH
			/* IsQuote() returns true for ' and "
			*  Make sure opening quote is equal to end quote.
			*/
			...
			else if (IsQuote(Ch) && bInQuote && Ch == quote)
			{
				bInQuote = false;
			}
			// end WITH_VH
