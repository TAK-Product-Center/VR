...
public:
	...
	CORE_API void AddBinariesDirectory(const TCHAR *InDirectory, bool bIsGameDirectory, bool bIsExtDirectory); // WITH_VH || Add bIsExtDirectory
	...
	*/
	...
	/**
	*	WITH_VH || Gets the external binaries directory
	*
	*	@return Returns array of external binaries directory
	*/
	TArray<FString> GetExtBinariesDirectory() const;

	/**
	*	WITH_VH || Checks to see if given file is external (outside of Game and Engine directory)
	*
	*   @param filePath A path to a file or directory, can be relative or absolute
	*   @return true if filePath is external
	*/
	bool IsExternalModulePath(const FString& filePath) const;
...
private:
	...
	/** WITH_VH || Array of external plugin binaries directories. */
	mutable TArray<FString> ExtBinariesDirectories;
	mutable TArray<FString> PendingExtBinariesDirectories;
