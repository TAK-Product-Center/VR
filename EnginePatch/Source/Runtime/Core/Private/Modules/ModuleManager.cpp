...
void FModuleManager::ResetModulePathsCache()
{
	...
	PendingExtBinariesDirectories.Append(MoveTemp(ExtBinariesDirectories)); // WITH_VH || add ext binaries directories variable.
}
...
void FModuleManager::FindModulePaths(const TCHAR* NamePattern, TMap<FName, FString> &OutModulePaths) const
{
	...
	// WITH_VH || Search any external plugin directories
	if (PendingExtBinariesDirectories.Num() > 0)
	{
		TArray<FString> LocalPendingExtBinariesDirectories = MoveTemp(PendingExtBinariesDirectories);
		check(PendingExtBinariesDirectories.Num() == 0);

		for (const FString& ExtBinaryDirectory : LocalPendingExtBinariesDirectories)
		{
			FindModulePathsInDirectory(ExtBinaryDirectory, true, ModulePathsCache);
		}

		ExtBinariesDirectories.Append(MoveTemp(LocalPendingExtBinariesDirectories));
	}
	// end WITH_VH
...
void FModuleManager::FindModulePathsInDirectory(const FString& InDirectoryName, bool bIsGameDirectory, TMap<FName, FString>& OutModulePaths) const
{
	...
	{
		FModuleManifest Manifest;
		// WITH_VH (VTAK-only) | added SKIPCOMPATIBLE check
		if (FModuleManifest::TryRead(FModuleManifest::GetFileName(SearchDirectoryName, bIsGameDirectory), Manifest) && (FParse::Param(FCommandLine::Get(), TEXT("SKIPCOMPATIBLE")) || Manifest.BuildId == BuildId.GetValue() || SearchDirectoryName.Contains(TEXT("/Engine/Plugins/Bridge/"))))
		{
...
void FModuleManager::AddExtraBinarySearchPaths()
{
	if (!bExtraBinarySearchPathsAdded)
	{
		...
		for (const FString& RestrictedFolderName : RestrictedFolderNames)
		{
			...
			{
				AddBinariesDirectory(*RestrictedFolder, false, false); // WITH_VH || Add bIsExtDirectory
			}
		}
...
// WITH_VH || Add bIsExtDirectory
void FModuleManager::AddBinariesDirectory(const TCHAR *InDirectory, bool bIsGameDirectory, bool bIsExtDirectory)
{
	if (bIsGameDirectory)
	{
	...
	for (const TCHAR* RestrictedFolderName : RestrictedFolderNames)
	{
		...
		if (FPaths::DirectoryExists(RestrictedFolder))
		{
			AddBinariesDirectory(*RestrictedFolder, bIsGameDirectory, bIsExtDirectory);  // WITH_VH added bIsExtDirectory
		}
	}
}
...
// WITH_VH || Add getter for external binaries directory.
TArray<FString> FModuleManager::GetExtBinariesDirectory() const
{
	TArray<FString> extBinDir;

	if (ExtBinariesDirectories.Num())
	{
		extBinDir = ExtBinariesDirectories;
		return extBinDir;
	}
	if (PendingExtBinariesDirectories.Num())
	{
		extBinDir = PendingExtBinariesDirectories;
		return extBinDir;
	}
	return extBinDir;
}
...
// WITH_VH || Check to see if a given file path is external 
bool FModuleManager::IsExternalModulePath(const FString& filePath) const
{
	bool isExternalFilePath = false;
	...
	FString fullManifestPath = FPaths::ConvertRelativePathToFull(filePath);

	if (!fullManifestPath.StartsWith(engineDir) && !fullManifestPath.StartsWith(gameDir))
	{
		isExternalFilePath = true;
	}

	return isExternalFilePath;
}
// end WITH_VH
