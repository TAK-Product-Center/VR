...
enum class EPluginType
{
 ...
 // WITH_VH | Enum for why an external plugin is invalid and should not be loaded.
enum class EInvalidPluginReason
{
	/** Plugin was marked for deletion */
	MarkedForDeletion,

	/** Plugin version doesn't match project version */
	VersionMismatch,

	/** Unable to load plugin module */
	FailedToLoadModule,
};
// end WITH_VH
...
// WITH_VH | Data structure that stores information about invalid plugins.
struct FInvalidPlugin
{
	/** The name of this plug-in. */
	FString Name;
	...
	/** Path to descriptor on disk. */
	FString DescriptorPath;

	/** Plugin type */
	EPluginType PluginType;

	/** Reason why plugin is invalid */
	EInvalidPluginReason InvalidReason;

	/** Reason why plugin failed to load. This will be empty if the invalid reason is something other than FailedToLoadModule */
	FText FailedToLoadReason;

	/** Store a reference to the plugin object created. */
...
public:
	...
	*/ 
	...
	/** WITH_VH || Getter for discovered external plugin directories. */
	virtual TArray<FString> GetDiscoveredExtPluginPaths() = 0;

	/** WITH_VH || Getter for plugins that were invalid and not loaded. */
	virtual TSharedRef<TMap<FString, FInvalidPlugin>> GetInvalidExternalPlugins() = 0;

	/** WITH_VH || Removes already deleted invalid plugins from invalid plugins map. */
	virtual void CleanInvalidPluginMap() = 0;
