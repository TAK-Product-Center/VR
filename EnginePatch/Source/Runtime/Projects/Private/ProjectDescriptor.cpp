...
bool FProjectDescriptor::Read(const FJsonObject& Object, const FString& PathToProject, FText& OutFailReason)
{
	...
	/** WITH_VH || read project version and game. There may be project descriptors without project names or version fields
	 *, so provide a default string if the fields don't exist.
	 * */
	if (!Object.TryGetStringField(TEXT("ProjectName"), ProjectName))
	{
		ProjectName = "";
	}

	if (!Object.TryGetStringField(TEXT("ProjectVersion"), ProjectVersion))
	{
		ProjectVersion = "";
	}
	/** end WITH_VH */
...
void FProjectDescriptor::Write(TJsonWriter<>& Writer, const FString& PathToProject) const
{
	...
	/** WITH_VH */
	Writer.WriteValue(TEXT("ProjectName"), ProjectName);
	Writer.WriteValue(TEXT("ProjectVersion"), ProjectVersion);
	/** end WITH_VH */
