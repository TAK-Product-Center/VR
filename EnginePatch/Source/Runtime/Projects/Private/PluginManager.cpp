...
TArray<FString> FPluginManager::DiscoveredExtPluginPaths = TArray<FString>(); // WITH_VH || define static member variable.
TMap<FString, FInvalidPlugin> FPluginManager::InvalidPlugins = TMap<FString, FInvalidPlugin>(); // WITH_VH || define static member variable.
...
namespace PluginSystemDefs
{
	...
	/** WITH_VH || Returns the path to the external plugin directory config */
	static const FString pathToExtPluginConfig = FPaths::Combine(FString(FPlatformProcess::ApplicationSettingsDir()).Replace(TEXT("Epic/"), TEXT("")),
																 TEXT("VRTAK"), TEXT("ExtPlugins"), TEXT("ExtPluginConfig.ini"));
...
void FPluginManager::ReadAllPlugins(FDiscoveredPluginMap& Plugins, const TSet<FString>& ExtraSearchPaths)
{
	...
	if (Project != nullptr)
	{
		...
		// WITH_VH || Add the plugin directories from the external plugin directory config file.
#if UE_BUILD_SHIPPING || UE_BUILD_TEST || UE_BUILD_DEVELOPMENT   // NOTE: Adding Dev to allow testing of Dev builds for ChemBio multiplayer network profiling.  Can be removed later.  -Billy G
		// Get path to external plugin directories config file, or create it if it doesn't exist. 

		// TODO: decide if this should be available for editor builds as well. With editor builds, they can access external plugins by using
		// the AdditionalPluginDirectories field in the .uproject file instead of using this folder.
		DiscoveredExtPluginPaths = FindExternalPluginDirectories(PluginSystemDefs::pathToExtPluginConfig);

		for (const FString& pluginDir : DiscoveredExtPluginPaths)
		{
			// For each external plugin directory, ensure it is valid and search for plugins within it.
			if (FPaths::DirectoryExists(pluginDir))
			{
				ReadPluginsInDirectory(pluginDir, EPluginType::External, Plugins, ChildPlugins);
			}
		}
#endif 
		// end WITH_VH
		...
		{
			DiscoveredExtPluginPaths.AddUnique(Dir); // WITH_VH
...
void FPluginManager::CreatePluginObject(const FString& FileName, const FPluginDescriptor& Descriptor, const EPluginType Type, FDiscoveredPluginMap& Plugins, TArray<TSharedRef<FPlugin>>& ChildPlugins)
{
	...
	// WITH_VH
	if (Plugin->GetDescriptor().bIsSlatedForDeletion)
	{
		// This plugin was slated for deletion and shouldn't be loaded, so we'll save plugin info and return. Deletion will be handled in later subsystems.
		FInvalidPlugin invalidPlugin;
		invalidPlugin.Name = Plugin->GetName();
		invalidPlugin.BasePath = Plugin->GetBaseDir();
		invalidPlugin.DescriptorPath = Plugin->GetDescriptorFileName();
		invalidPlugin.PluginType = Type;
		invalidPlugin.InvalidReason = EInvalidPluginReason::MarkedForDeletion;
		invalidPlugin.Plugin = TSharedPtr<IPlugin>(Plugin);

		InvalidPlugins.Add(Plugin->GetName(), invalidPlugin);
		return;
	}

	if (Plugin->Type == EPluginType::External)
	{
		// Limit this check to external plugins only.
		const FProjectDescriptor* ProjectDesc = IProjectManager::Get().GetCurrentProject();
		if (ProjectDesc)
		{
			if (Plugin->GetDescriptor().VersionName != ProjectDesc->ProjectVersion)
			{
				// Mismatch in version name, do not load it. 
				FInvalidPlugin invalidPlugin;
				invalidPlugin.Name = Plugin->GetName();
				invalidPlugin.BasePath = Plugin->GetBaseDir();
				invalidPlugin.DescriptorPath = Plugin->GetDescriptorFileName();
				invalidPlugin.PluginType = Type;
				invalidPlugin.InvalidReason = EInvalidPluginReason::VersionMismatch;
				invalidPlugin.Plugin = TSharedPtr<IPlugin>(Plugin);
...
// WITH_VH | add getter for discovered external plugin paths
TArray<FString> FPluginManager::GetDiscoveredExtPluginPaths()
{
	return DiscoveredExtPluginPaths;
}
// end WITH_VH

// WITH_VH | add getter for list of invalid external plugins
TSharedRef<TMap<FString, FInvalidPlugin>> FPluginManager::GetInvalidExternalPlugins()
{
	return MakeShared<TMap<FString, FInvalidPlugin>>(InvalidPlugins);
}
// end WITH_VH

// WITH_VH | removes already deleted invalid plugins from invalid plugins map
void FPluginManager::CleanInvalidPluginMap()
{
	TArray<FString> removedInvalidPlugins;
	for (const TPair<FString, FInvalidPlugin>& invalidPluginPair : InvalidPlugins)
	{
		FInvalidPlugin invalidPlugin = invalidPluginPair.Value;
		if (!FPaths::DirectoryExists(invalidPlugin.BasePath))
		{
			removedInvalidPlugins.Add(invalidPluginPair.Key);
		}
	}

	for (const FString& removedPluginName : removedInvalidPlugins)
	{
		InvalidPlugins.FindAndRemoveChecked(removedPluginName);
	}
}
// end WITH_VH
...

		{
			...
			for (TPair<FString, FPlugin*>& Pair : EnabledPlugins)
			{
				...
				if (Plugin.Descriptor.Modules.Num() > 0)
				{
					...
					FModuleManager::Get().AddBinariesDirectory(*PluginBinariesPath, 
															   Plugin.GetLoadedFrom() == EPluginLoadedFrom::Project,
															   Plugin.GetType() == EPluginType::External); // WITH_VH | Add bIsExtDirectory
				}
...
bool FPluginManager::TryLoadModulesForPlugin( const FPlugin& Plugin, const ELoadingPhase::Type LoadingPhase ) const
{
	...
	for( auto FailureIt( ModuleLoadFailures.CreateConstIterator() ); FailureIt; ++FailureIt )
	{
		...
		if( FailureReason != EModuleLoadResult::Success )
		{
			...
			const FText PluginPathText = FText::FromString(Plugin.GetBaseDir()); // WITH_VH - add better failure messages to indicate where plugins are being loaded from.
			const FText TextModuleName = FText::FromName(FailureIt.Key());

			if (FailureReason == EModuleLoadResult::FileNotFound)
			{
				// WITH_VH - add PluginPathText to indicate where plugins are being loaded from.
				FailureMessage = FText::Format(LOCTEXT("PluginModuleNotFound", "Plugin '{0}' in path '{1}' failed to load because module '{2}' could not be found.  Please ensure the plugin is properly installed, otherwise consider disabling the plugin for this project."), PluginNameText, PluginPathText, TextModuleName);
			}
			else if (FailureReason == EModuleLoadResult::FileIncompatible)
			{
				// WITH_VH - add PluginPathText to indicate where plugins are being loaded from.
				FailureMessage = FText::Format(LOCTEXT("PluginModuleIncompatible", "Plugin '{0}' in path '{1}' failed to load because module '{2}' does not appear to be compatible with the current version of the engine.  The plugin may need to be recompiled."), PluginNameText, PluginPathText, TextModuleName);
			}
			else if (FailureReason == EModuleLoadResult::CouldNotBeLoadedByOS)
			{
				// WITH_VH - add PluginPathText to indicate where plugins are being loaded from.
				FailureMessage = FText::Format(LOCTEXT("PluginModuleCouldntBeLoaded", "Plugin '{0}' in path '{1}' failed to load because module '{2}' could not be loaded.  There may be an operating system error or the module may not be properly set up."), PluginNameText, PluginPathText, TextModuleName);
			}
			else if (FailureReason == EModuleLoadResult::FailedToInitialize)
			{
				// WITH_VH - add PluginPathText to indicate where plugins are being loaded from.
				FailureMessage = FText::Format(LOCTEXT("PluginModuleFailedToInitialize", "Plugin '{0}' in path '{1}' failed to load because module '{2}' could not be initialized successfully after it was loaded."), PluginNameText, PluginPathText, TextModuleName);
			}
			else
			{
	...
	{
		// WITH_VH | handle errors in external plugin loading
		if (Plugin.Type == EPluginType::External)
		{
			FInvalidPlugin invalidPlugin;
			invalidPlugin.Name = Plugin.Name;
			invalidPlugin.PluginType = Plugin.Type;
			invalidPlugin.BasePath = Plugin.GetBaseDir();
			invalidPlugin.DescriptorPath = Plugin.GetDescriptorFileName();
			invalidPlugin.InvalidReason = EInvalidPluginReason::FailedToLoadModule;
			invalidPlugin.FailedToLoadReason = FailureMessage;
			//invalidPlugin.Plugin = TSharedPtr<IPlugin>(AllPlugins[Plugin.Name]);  // todo ohg 5.3.2

			InvalidPlugins.Add(Plugin.Name, invalidPlugin);
...
bool FPluginManager::LoadModulesForEnabledPlugins( const ELoadingPhase::Type LoadingPhase )
{
	...
	else
	{
		...
		TArray<FString> externalPluginsFailedToLoad;  // WITH_VH
		...
		for (const FDiscoveredPluginMap::ElementType& PluginPair : AllPlugins)
		{
			...
			if (Plugin->bEnabled && !Plugin->Descriptor.bExplicitlyLoaded)
			{
				...
				{
					// WITH_VH | keep loading plugins if an external plugin fails to load */
					if (Plugin->Type == EPluginType::External)
					{
						externalPluginsFailedToLoad.Add(Plugin->Name);
					}
					else
					{
						bSuccess = false;
		...
		// WITH_VH | remove references to external plugins that failed to load
		for (const FString& pluginName : externalPluginsFailedToLoad)
		{
			AllPlugins.FindAndRemoveChecked(pluginName);
		}
		// end WITH_VH
...
/** WITH_VH || Looks into an external plugin .ini file and attempts to load external plugins in directories listed in 
  * section ExtPluginDirectories.
  * If the .ini file doesn't exist, it'll create both the .ini file and a default external plugin directory.
  */
TArray<FString> FPluginManager::FindExternalPluginDirectories(const FString& configFile)
{
	TArray<FString> externalPluginDirectories;
	// Make a temporary FConfigCacheIni that will be deleted at the end of this function.
	FConfigCacheIni* temporaryConfig = new FConfigCacheIni(EConfigCacheType::DiskBacked);

	// If it isn't valid, we should create it and populate it with a default external plugin directory.
	if (!FPaths::FileExists(configFile))
	{
		IPlatformFile& platformFile = FPlatformFileManager::Get().GetPlatformFile();

		// Create a default directory to house external plugins in. It will be in the same directory as the plugin config file.
		FString extPluginConfigPath = FPaths::GetPath(configFile) / TEXT("ExtPluginDirectories");
		extPluginConfigPath = FPaths::ConvertRelativePathToFull(extPluginConfigPath);

		bool isSuccess = platformFile.CreateDirectoryTree(*extPluginConfigPath);

		if (!isSuccess)
		{
			UE_LOG(LogPluginManager, Warning, TEXT("FindExternalPluginDirectories() || Was not able to create default external plugin directory, exiting early."));
			return externalPluginDirectories;
		}

		// Create config file and write the default external plugin directory path into it.
		FConfigFile* extPluginConfig = temporaryConfig->Find(configFile);

		if (extPluginConfig == nullptr)
		{
			UE_LOG(LogPluginManager, Warning, TEXT("FindExternalPluginDirectories() || Was not able to create external plugin config file, exiting early."));
			return externalPluginDirectories;
		}

		extPluginConfig->SetString(TEXT("ExtPluginDirectories"),
			TEXT("Path"),
			*extPluginConfigPath);
	}

	temporaryConfig->GetArray(TEXT("ExtPluginDirectories"),
		TEXT("Path"),
		externalPluginDirectories,
		configFile);

	if (externalPluginDirectories.Num() == 0)
	{
		UE_LOG(LogPluginManager, Log, TEXT("FindExternalPluginDirectories() || Zero plugin directories found."));
	}

	// Delete cache after usage.
	temporaryConfig->Exit();
	delete temporaryConfig;
	temporaryConfig = nullptr;
