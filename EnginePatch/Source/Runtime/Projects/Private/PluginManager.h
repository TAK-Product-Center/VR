...
public:
	...
	virtual TArray<FString> GetDiscoveredExtPluginPaths(); // WITH_VH | getter for discovered external plugin paths.
	virtual TSharedRef<TMap<FString, FInvalidPlugin>> GetInvalidExternalPlugins(); // WITH_VH | getter for external plugins that were invalid and not loaded.
	virtual void CleanInvalidPluginMap(); // WITH_VH | removes already deleted invalid plugins from invalid plugins map.
...
private:
	...
	/** WITH_VH | Look into .ini file and return a list of external plugin directories */
	static TArray<FString> FindExternalPluginDirectories(const FString& configFile);
...
private:
	...
	/** WITH_VH | External plugins that are invalid and were not loaded. */
	static TMap<FString, FInvalidPlugin> InvalidPlugins;
	...
	/** WITH_VH | List of discovered external plugin directory paths */
	static TArray<FString> DiscoveredExtPluginPaths;
