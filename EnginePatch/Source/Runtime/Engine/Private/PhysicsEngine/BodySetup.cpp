...
// WITH_VH
bool UBodySetup::IsReadyForFinishDestroy()
{
	if (CurrentCookHelper != nullptr)
	{
		CurrentCookHelper->CancelCookAsync();
		CurrentCookHelper = nullptr;
	}

	return Super::IsReadyForFinishDestroy() && (CurrentCookHelper == nullptr || !CurrentCookHelper->HasWork());
}
// end WITH_VH
