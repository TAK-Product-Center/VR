...
void UVoiceChannel::ReceivedBunch(FInBunch& Bunch)
{
	if (Connection->Driver && Connection->Driver->World)
	{
		while (!Bunch.AtEnd())
		{
			...
			OnPacketReceived.Broadcast(VoicePacket);  // WITH_VH
		}
	}
}
...
void UVoiceChannel::Tick()
{
	...
	if (bHandshakeCompleted)
	{
		...
		{
			//if (Connection->IsNetReady(0) == false) // WITH_VH
			if (!Connection->IsNetReady(true) || NumOutRec >= (RELIABLE_BUFFER / 4))  // WITH_VH
			{
			...
			//Bunch.bReliable = OpenAcked == false || Packet->IsReliable();  // WITH_VH
			Bunch.bReliable = true; // WITH_VH
			...
			if (Bunch.IsError() == false)
			{
				...
				OnPacketSent.Broadcast(Packet);  // WITH_VH
			}
			else
			{
