...
FReply FSceneViewport::OnMouseButtonDown( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
	{
		...
		// WITH_VH
		bool bLeftClickWhenCaptureRight = ViewportClient->GetMouseCaptureMode() == EMouseCaptureMode::CaptureDuringRightMouseDown && InMouseEvent.GetEffectingButton() == EKeys::LeftMouseButton;
		// end WITH_VH

		// Process the mouse event
		if (bTemporaryCapture || bProcessInputPrimary || bLeftClickWhenCaptureRight)  // WITH_VH added bLeftClickWhenCaptureRight
		{
