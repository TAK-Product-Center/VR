...
// WITH_VH (VTAK-only)
void FCurlHttpRequest::CustomizeForVH(const TMap<FString, FString>& keyValues)
{
	if (EasyHandle != nullptr)
	{
		...
		const FString* certPtr = keyValues.Find(TEXT("cert"));
		if (certPtr != nullptr)
		{
			curl_easy_setopt(EasyHandle, CURLOPT_SSLCERT, TCHAR_TO_ANSI(*(*certPtr)));
		}

		const FString* keyPtr = keyValues.Find(TEXT("key"));
		if (keyPtr != nullptr)
		{
			curl_easy_setopt(EasyHandle, CURLOPT_SSLKEY, TCHAR_TO_ANSI(*(*keyPtr)));
		}

		const FString* caCertPtr = keyValues.Find(TEXT("caCert"));
		if (caCertPtr != nullptr)
		{
			curl_easy_setopt(EasyHandle, CURLOPT_CAINFO, TCHAR_TO_ANSI(*(*caCertPtr)));
		}
	}
}
// end WITH_VH (VTAK-only)
