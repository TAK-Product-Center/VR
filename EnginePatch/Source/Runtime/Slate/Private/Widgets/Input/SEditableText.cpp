...
FReply SEditableText::OnKeyUp( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent )
{
	...
	/* WITH_VH
	* Unmodified returns EditableTextLayout->HandleKeyUp(InKeyEvent);
	* Original implementation returned FReply::Unhandled() unless virtual keyboard was required in EditableTextLayout
	* Returning FReply::Unhandled() causes blueprint input events to fire OnKeyUp, which is not expected behavior.
	*/
	return FReply::Handled();
}
