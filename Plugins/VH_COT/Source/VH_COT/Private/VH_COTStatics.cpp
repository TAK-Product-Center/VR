...
bool UVH_COTStatics::IsPolylineClosed(UVH_COTEvent* cotEvent)
{
    TArray<FXmlNode*> links = cotEvent->GetNodes(TEXT("link"));

	if (links.Num() > 0)
	{
		return LinkToCoordinate(links[0]) == LinkToCoordinate(links[links.Num() - 1]);
	}

	return false;
}

void UVH_COTStatics::SetPolylineClosed(UVH_COTEvent* cotEvent, bool bClosed)
{
	bool bCurrentlyClosed = IsPolylineClosed(cotEvent);

	TArray<FXmlNode*> links = cotEvent->GetNodes(TEXT("link"));

	if (bCurrentlyClosed && !bClosed)
	{
		links.RemoveAt(links.Num() - 1);
	}
	else if (!bCurrentlyClosed && bClosed)
	{
		TArray<FXmlNode*> detail = cotEvent->GetNodes(TEXT("detail"));
		if (detail.Num() > 0)
		{
			detail[0]->AppendChildNode(TEXT("link"), TEXT(""), links[0]->GetAttributes());

			links = cotEvent->GetNodes(TEXT("link"));
		}
	}
	...
	{
		cotEvent->AddNode(TEXT("link"), link->GetAttributes());
	}
}

FString UVH_COTStatics::SanitizeRawCOTAttributes(const FString& inCot)
{
